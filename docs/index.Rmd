...
m2022 <- mapview::mapview(
  dd_2022, zcol = "Log_accesses",
...
## Chapter 4: Distribution of accesses to AVCs
...
m2024 <- mapview::mapview(
  dd_2024, zcol = "Log_accesses",
...
$$
  ...
  - $r$: **temporal** autocorrelation; $\sigma^2$: **scale** parameter;
...
## Chapter 4: Penalised Complexity priors
  ...
  - **Interpretation**: the parameter exceeds a **threshold value** with a **fixed probability** *a priori*
  ...
  - $r$, $\sigma$, $\phi$: PC-prior already developed [@Sorbye2017], [@PC] $\Rightarrow$ Gaps we fill:
    ...
    - **LCAR** model: PC-prior for $\lambda$ (spatial dependence/prec. mixing)
    - General contribution: PC-prior for $\rho$, $\lambda$ and $\phi$ in the multivariate case [@Mmod]
