...
```{r transport map prov, echo = FALSE, message = FALSE, warning = FALSE, output = FALSE, fig.height = 3.2 }
...
```{r transport map mun, echo = FALSE, message = FALSE, warning = FALSE, output = FALSE, fig.height = 3.2 }
library(SchoolDataIT)
...
## Chapter 4: The regression model
  ...
  - Standard **Poisson** linear regression: for $i \in \lbrace 1, 2, \dots, 256 \rbrace$, and $t \in \lbrace 2021, 2022, 2023, 2024 \rbrace$ we have
...
$$\eta_{it} := \boldsymbol{x_{it}}^\top \beta + z_{it}$$
  ...
  - $\boldsymbol{Z}$: spatio-temporal latent effect of size $n \times k$, where $n=256$ and $k=4$;
...
## Chapter 4: Penalised Complexity priors
  ...
  - **Gap we fill**: PC-prior for **spatial association parameters** in the general, **multivariate** case [@Mmodels], including the **Besag-York-Mollié** [@BYM2] $\Rightarrow$ univariate prior on $\rho$ (PCAR) and $\lambda$ (LCAR) follows **by deduction**
