...
## Chapter 3: why the Skew-Normal? Resudual density KDE{.pic-slide-small}
...
$$\eta_{it} := X_{it}^\top \beta + Z_{it}$$
  ...
  - $Z_{it}$: spatio-temporal latent effect of size $n \times k$, where $n=256$ and $k=4$;
    ...
    - Spatial structure: **four** alternative models
...
$$
  ...
  $\boldsymbol{\Omega}$: model-spacific **spatial covariance** matrix
  
    - ICAR: $\boldsymbol{\Omega} = \boldsymbol{L^+}$; $\boldsymbol{L}$=graph Laplacian
...
## Chapter 4: Penalised Complexity priors
  ...
  - $r$, $\sigma$, $\phi$: PC-prior already developed [@Sorbye2017], [@PC] $\Rightarrow$ **Gaps** we fill:
    - **PCAR** model: PC-prior for $\rho$ (spatial autocorrelation)
    - **LCAR** model: PC-prior for $\lambda$ (spatial dependence)
    - General contribution: PC-prior for $\rho$, $\lambda$ and $\phi$ in the multivariate case [@Mmodels]
