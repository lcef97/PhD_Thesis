...
In this chapter, we provide an overview on the statistical methodology followed throughout the following chapters. In particular, this chapter is intended as a minimal toolbox in areal data modelling, with specific application in chapter \ref{Chapter:Invalsi}.
We start by introducing perhaps the simplest and most intuitive model in the class of multivariate autoregressives, namely the Intrinsic Conditional Autoregressive \citep{Mardia}, which generalises to the multivariate case the seminal paper of \cite{CAR}.

%% -------- ICAR ----------------------------------------- %%
\section{The intrinsic CAR model} \label{section:icar}

Consider a $p$-variate random variable $z_i = (z_{i,1},   \, \ldots, \, z_{i,p})'$ taking values over the $i$-th node of a graph $\mathcal{G}=(\mathcal{S}, \mathcal{E})$ where $\mathcal{E}$ is the set of edges and $\mathcal{S}$ the set of nodes (sites) $\mathcal{S} \subset \mathbb{N}^n$. The graph is described by:
\begin{itemize}
\item The binary neighbourhood matrix $\mathbf{W}$, with $w_{ij} = 1 \Longleftrightarrow s_i \in \partial s_j$ and $w_{ij}$ otherwise; notice that by convention, $w_{ii}=0$ (cfr Chapter \ref{section:GMRFs})
...
\begin{equation}
\pi(z  | \Lambda) \propto -\frac{1}{2} \mathrm{vec}_r(z) ^{\top} (\mathbf{L} \otimes \Lambda)\, \mathrm{vec}_r(z)
\label{eq:rowwise}
\end{equation}
...
%
Considering that$
\mathrm{Var}[\mathrm{vec}_c(z)] = \mathcal{P} \times \mathrm{Var}[\mathrm{vec}_r(z)] \times \mathcal{P}^{\top}$, hence $\mathrm{Prec}[\mathrm{vec}_c(z)] = \mathcal{P} \times \mathrm{Prec}[\mathrm{vec}_r(z)] \times \mathcal{P}$, it holds that:
$$
\mathcal{P} \left( \mathbf{L} \otimes \Lambda \right) \mathcal{P}^{\top}= \Lambda \otimes \mathbf{L}
$$
...
%
\begin{equation}
z|\Lambda \sim \mathcal{N} \left( 0, (\Lambda \otimes \mathbf{L})^{+}\right)
\end{equation}
%
The $^{+}$ superscript here denotes the Moore-Penrose pseudoinverse, which is the block-component of the variance of intrinsic models such as the ICAR.
...
or typical marginal variance). We are thus able to separate the parameter $\Lambda$ from the effect on precision induced by the graph structure. To do so, the scaled model does not employ $\mathbf{L}$ as the structure matrix, but the scaled matrix $\mathbf{L}_\mathrm{scaled} = \bar{\sigma}^2 \mathbf{L} $. On the other hand, the precision parameter becomes $ \displaystyle{\frac{1}{\bar{\sigma}^2}} \Lambda$, and this is what actually expresses the precision of the process independently of how the graph is structured. 
...
$$
Where $\mathcal{G}$ is an appropriate permutation matrix, $\mathbf{L_i}$ is the Laplacian of the $i$-th component of the graph, and $\bar{\sigma}_i^2$ is the relevant typical variance.


%% -------- LGM outline ---------------------------------- %%
\section{Latent Gaussian model outline}\label{section:LGM}
...
Autoregressive models are usually employed as latent variables in spatial regression. This is partly motivated by the improper prior distribution of intrinsic processes, which however does not imply an improper posterior \citep{Banerjee}. Here we briefly outline a general latent Gaussian model, which will be useful to explain the next section. 
...
And
\begin{equation}
E[z | y, \tau_{\varepsilon}, \bar{r}] = \left[ S_{11} \right]^{-1}
...
\begin{equation}
z_{i} | z_{-i}, \Lambda \sim \mathcal{N} \left(\sum_{j \sim i} \frac{w_{ij}}{d_i} z_{j}, \, \frac{1}{d_i} \Lambda^{-1}\right)
\label{eq:ICAR_local}
\end{equation}
