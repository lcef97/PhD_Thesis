...
In this chapter, we provide an overview on the statistical methodology followed throughout the following chapters. In particular, this chapter is intended as a minimal toolbox in areal data modelling, with specific application in chapter \ref{Chapter:Invalsi}. Since the application regards multivariate data, here we describe multivariate models directly, although the univariate case is perhaps more frequent in application literature. However, once the multivariate model is defined, the univariate one follows straightforwardly. 

We start by introducing the simplest and most intuitive autoregressive model, namely the Intrinsic Conditional Autoregressive \citep{Mardia}, which generalises to the multivariate case the seminal paper of \cite{CAR}. We then introduce some simple extensions of the ICAR which may be needful to handle random noise. 

Lastly, we introduce a typical and frequently discussed issue not only in areal data modelling but in spatial statistic literature as a whole, i.e. spatial confounding. 
...
%
$$
\mathrm{VAR}[z_{ih} \mid \Sigma] = \sigma_{h}^2 s_{ii}
$$
Where $\sigma_h^2$ is the marginal variance of the $h$-th variable, corresponding to the $h$-th diagonal element of of $\Sigma$; in other words, $s_{ii}$ would be the marginal variance of $z_i$ if the relevant global precision was equal to 1.
%
...
To gain more insight on restricted regression, let us briefly outline a general latent Gaussian linear model, which will be useful to explain the current section, and consider:
$$
\left\{ \begin{array}{ll} 
...
\end{equation}
%
Where $\tau_\varepsilon \in \psi$ is the error precision parameter and $N$ is an arbitrarily large scalar.
...
\end{equation}
%
Where  $\boldsymbol{A_1} = \left(\boldsymbol{X} \, \boldsymbol{\xi} \right)$, $\vartheta_1 = (\beta^{\top} z^{\top})^{\top}$ is the vector of parameters, and $n_\vartheta = n_z + p$. The term $\boldsymbol{A_2} \vartheta_2$ has by construction both mean and variance zero. Moreover,
$$ 
...
\end{aligned}
\end{equation}
Where $\boldsymbol{S_{11}} := (\boldsymbol{\xi} ^{\top} \boldsymbol{\xi} + \bar{r} \boldsymbol{R} - \boldsymbol{\xi} ^{\top} \boldsymbol{X G}^{-1} \boldsymbol{X}^{\top}\boldsymbol{\xi})$ is the Schur complement of the upper-left block in $\left(\boldsymbol{A_1}^\top \boldsymbol{A_1} + \boldsymbol{C}^{-1}\right)^{-1}$ \citep[][paragraph 0.7.3]{HornJohnson}, $\bar{r} := \dfrac{\tau}{\tau_{\varepsilon}}$ and $\boldsymbol{G} := (\boldsymbol{X}^{\top}\boldsymbol{X} + r_{\varepsilon}\boldsymbol{I_p})$, where $r_{\varepsilon} := \dfrac{1}{\tau_{\varepsilon}N} = \dfrac{\sigma_{\varepsilon}^{2}}{N}$.
...
Equivalently, if for brevity we define $\boldsymbol{\Gamma}= \boldsymbol{\xi} ^{\top} \boldsymbol{\xi} + \bar{r} \boldsymbol{R} - \boldsymbol{\xi}^{\top} \boldsymbol{X G}^{-1} \boldsymbol{X}^{\top} \boldsymbol{\xi}$ this means that the hat matrix equals:
\begin{equation}
\begin{aligned}
...
\label{eq:projmat}
\end{equation}
Where the subscript $\tau_e, r$ denotes the dependence on the two precision parameters. If a flat prior is defined on $\beta$, we have $M \longrightarrow \infty$, with $G \longrightarrow X^{\top}X$. In this case, it is possible to show that the limits for equations \ref{eq:expbeta} and \ref{eq:expzeta} are the results stated by \cite[][page 1199]{RHZ} and the hat matrix does not depend on $\tau_{\varepsilon}$ but only on $\bar{r}$.
...
Let us partition the eigenvector matrix of $P_{\bot}$ into $(E \, K)$; $E$ are the first $n-p$ eigenvectors corresponding to nonzero eigenvalues, while $K$ belongs to the null space of $P_{\bot}$. Thus, the decomposition of $P_{\bot}$ is:

$$
P_{\bot} =  \begin{pmatrix}E & K \end{pmatrix} 
\begin{pmatrix} I_{n-p} & 0_{(n-p) \times p } \\ 0_{p \times (n-p)} & 0_{p \times p} \end{pmatrix}
 \begin{pmatrix} E^{\top} \\ K^{\top} \end{pmatrix} = E E^{\top}
$$

Notice that:
$$
\begin{pmatrix} E^{\top} \\ K^{\top}\end{pmatrix} (E \, K) = (E \, K) \begin{pmatrix} E^{\top} \\ K^{\top}\end{pmatrix} = I_n \, , \quad KK^{\top} = P, \, \quad K^{\top}E = 0 
$$

Without loss of generality, it holds that
$$
\ln \pi(z | \tau) = C -\dfrac{\tau}{2} z^{\top} \boldsymbol{R} z  = 
C -\dfrac{\tau}{2} z^{\top} (E \, K) \begin{pmatrix} E^{\top} \\ K^{\top}\end{pmatrix} 
\boldsymbol{R} (E \, K) \begin{pmatrix} E^{\top} \\ K^{\top}\end{pmatrix} z 
$$

Where $C$ is, as usual, an additive constant. Let us now define $\zeta = \begin{pmatrix} E^{\top} \\ K^{\top} \end{pmatrix} z$, such that $z = (E \, K) \zeta$ and decompose the newly defined spatial effect $\zeta$ as
$$
\zeta = \begin{pmatrix} \zeta_1 \\ \zeta_0 \end{pmatrix} \quad \text{where} \quad  \zeta_1 := E^{\top}z \quad \text{and} \quad \zeta_0 := K^{\top}z
$$ 
The distribution of the constrained latent field then reads as:
$$
\zeta \sim \mathcal{N} \left(\begin{pmatrix} 0 \\ 0 \end{pmatrix}, \, \tau^{-1} \begin{pmatrix} E^{\top}\boldsymbol{R}E & E^{\top}\boldsymbol{R}K \\ K^{\top}\boldsymbol{R}E & K^{\top}\boldsymbol{R}K \end{pmatrix}^{-1} \right)
$$

If the random effect $z$ displays multicollinearity with the covariates $X$, we are in presence of spatial confounding. Restricted spatial regression, then, works by constraining $z$ onto the null space of $P$, which is orthogonal to $X$: 
$$
\mathrm{RSR}: \zeta_0 = K^{\top}z = 0
$$
Specifically, the regression model under RSR reads as:

\begin{equation} \label{eq:RSR}
\mathrm{RSR}: \quad y = X \beta + P_{\bot}z + \varepsilon = X \beta + E \zeta + \varepsilon
\end{equation}
...
With respect to equation \ref{eq:RSR}, if we consider $\xi = E$ and $\vartheta_1^{\top} = (\beta^{\top}, (E^{\top}z)^{\top})$, it is straightforward to see that $\mathbb{E}[\beta|y, \tau_{\varepsilon}, \bar{r}] = G^{-1} X^{\top}y$; if $p(\beta)$ is flat, its posterior expectation reduces to $X^{\top}(X^{\top}X)^{-1}y$, which is the ordinary least squares estimator.

As a side effect of deconfounding $z$, it may be worth noticing that the the size of the new precision matrix is $(n-p) \times (n-p)$ and its rank is $ \leq \mathrm{min} \lbrace rk(E), rk(\boldsymbol{R}) \rbrace = \mathrm{min} \lbrace (n-p), rk(\boldsymbol{R}) \rbrace$. Hence, if the rank deficiency of $\boldsymbol{R}$ is less than $p$, the new spatial effect has a proper distribution even if $z$ is an ICAR process. If the underlying graph is disconnected and $p(z)$ can be factorised in a factor for each connected component (CC) then the rank deficiency of $\boldsymbol{R}$ is equal to the number of CCs, but it is necessary to impose a sum to zero constraint for each CC, hence the same number of intercepts would be required; thus, also in this case $n-p$ is smaller than $rk(\boldsymbol{R})$ and $E^{\top}\boldsymbol{R}E$ is still proper. However, there are still some possible situations in which this inconvenience can arise, e.g. in multilevel models, like in Chapter \ref{Chapter:Invalsi}.
...
\subsubsection{Spatial+: Filtering out spatial variability from covariates}
...
An variant of Spatial+ has recently been proposed to circumvent the specification of a nested parametric model on covariates. In line with \citep{Lamouroux}, we label this method as \textit{Spatial+2.0}.
...
The \texttt{SchoolDataIT} software is currently available under version 0.2.8, released on October 2nd 2025 on the Comprehensive R Archive Network \href{https://cran.r-project.org/web/packages/SchoolDataIT/index.html}{(CRAN)}. To ensure constant package maintenance, experimental versions are hosted on \href{https://github.com/lcef97/SchoolDataIT}{my GitHub webpage}.
...
For an appropriately chosen $n \times 4$ matrix $\boldsymbol{b}$, $\boldsymbol{\bar{X}}$ can be expressed as a linear combination of $\boldsymbol{V}$:
$$
\boldsymbol{\bar{X}} = \boldsymbol{Vb}
$$
Intuitively, the spatial component of $\boldsymbol{\bar{X}}$ is determined by the last columns of $\boldsymbol{V}$ \citep{Urdangarin24}: without loss of generality, $\boldsymbol{\bar{X}}$ is decomposed into:
$$
\boldsymbol{\bar{X} }= \boldsymbol{\bar{X}^{(NS)}} + \boldsymbol{\bar{X}^{(S)}} +\boldsymbol{ \bar{X}^{(0)}}
$$
where $\boldsymbol{\bar{X}^{(NS)}}$ is the nonspatial component, given by the linear combination of the first $n-G-K$ eigenvectors (with $G=3$ connected graph components), $\boldsymbol{\bar{X}^{(s)}}$ is the combination of the eigenvectors associated with the last $K$ nonzero eigenvalues and represents the spatial component, and $\boldsymbol{\bar{X}^{(0)}}$ is the combination of the $G=3$ eigenvectors in the null space of the Laplacian matrix, constant within each connected component. 
To remove spatial confounding, it is sufficient to consider $\xi \left(\boldsymbol{\bar{X}^{(NS)}} \right) + \boldsymbol{ \Delta X}$ as the covariate matrix in the regression model. 
