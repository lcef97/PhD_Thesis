...
Consider a $k$-variate random variable $z_i = (z_{i,1},   \, \ldots, \, z_{i,p})'$ taking values over the $i$-th node of a graph $\mathcal{G}=(\mathcal{S}, \mathcal{E})$ where $\mathcal{E}$ is the set of edges, such that a generic element $\epsilon_{ij}$ denotes the edge between sites $s_i$ and $s_j$;  $\mathcal{S} := \lbrace s_1, s_2, \dots s_n \rbrace$ the set of nodes (sites), and $\partial_i := \lbrace s_j :\epsilon_{ij} \in \mathcal{E}\rbrace$ is the set of neighbours of site $i$.  The graph can be described by:
\begin{itemize}
\item The binary neighbourhood matrix $\boldsymbol{W}$, with $w_{ij} = 1 \Longleftrightarrow s_i \in \partial s_j$ and $w_{ij}=0$ otherwise; notice that by convention, $w_{ii}=0$ (cfr Chapter \ref{section:GMRFs})
\item The diagonal degree matrix $\boldsymbol{D}$, whose elements correspond to the number of edges of each node or, alternatively, to the row or column sums of $\boldsymbol{W}$, hence $d_i = \sum_{j=1}^{n} w_{ij} = \mathrm{card}(\partial s_i)$
...
%
\begin{align}
\ln \left(z_i | \lbrace x_j \rbrace_{j<i} \, \cap \, \lbrace z_k \rbrace_{k>i}, \Sigma  \right) = \\ =
C_0 - \frac{d_i}{2} \left( 
z_i - \sum_{j=1}^{i-1} \frac{w_{ij}}{d_i} x_j  - \sum_{k=i+1}^{n} \frac{w_ik}{d_i}z_k
\right)^{\top} \Sigma^{-1} \left( 
z_i - \sum_{j=1}^{i-1} \frac{w_{ij}}{d_i} x_j  - \sum_{k=i+1}^{n} \frac{w_ik}{d_i}z_k
\right)
\end{align}
%
Where $C_0$ is an additive constant. Now, with no loss of generality, let us assume $x_i = 0 \,   \forall \, i \in [1,n]$, so that numerator elements in equation \ref{eq:brookslemma} can be written as
%
\begin{align}
 \ln \pi \left(z_i | \lbrace x_j =0\rbrace_{j<i} \, \cap \, \lbrace z_k \rbrace_{k>i}, \Sigma  \right) =
 C_0 - \frac{d_i}{2 } \left(z_i   -\sum_{k=i+1}^n \frac{w_{ij}}{d_i}z_k  \right) ^{\top}
 \Sigma^{-1}  \left(z_i   -\sum_{k=i+1}^n \frac{w_{ij}}{d_i}z_k  \right)=  \\
 = C_0- \frac{d_i}{2 } \left[ z^{\top}_i \Sigma^{-1} z_i - 
 2 \left(\sum_{k=i+1}^n    \frac{w_{ik}}{d_i}z_k\right) ^{\top} \Sigma^{-1} z_i \right] 
 - \frac{d_i}{2}  \left( 
...
%
\begin{equation}
 \ln \pi \left(x_i | \lbrace x_j =0\rbrace_{j<i} \, \cap \, \lbrace z_k \rbrace_{k>i}, \Sigma  \right)\mid_{x_i=0} = C_0 - \frac{d_i}{2}  \left( 
 \sum_{k=i+1}^n \frac{w_{ik}}{d_i} z_k
 \right) ^{\top} \Sigma^{-1} \left( 
 \sum_{k=i+1}^n \frac{w_{ik}}{d_i} z_k
 \right)
...
%
\begin{equation}
\ln \pi(z \mid \Sigma) = C_1 + \sum_{i=1}^n d_i z_i ^{\top} \Sigma^{-1} z_i -
\sum_{i=1}^n \sum_{k=1}^n w_{ik} z_i^{\top} \Sigma^{-1} z_k
\end{equation}
%
...
$$
 \sum_{i=1}^n d_i z^{\top}_i \Sigma^{-1} z_i = 
\begin{pmatrix}
z_{11} \\ \vdots \\ z_{1k} \\ z_{21} \\ \vdots \\ z_{2k} \\ \vdots \\z_{n1} \\ \vdots \\ z_{nk}
\end{pmatrix}^{\top}
\begin{pmatrix}
d_1 \Sigma^{-1} & 0 & \ldots & 0 \\
0 & d_2 \Sigma^{-1} & \ldots & 0 \\
\vdots & \vdots & \ddots & \vdots \\
0 & 0 & \ldots & d_n \Sigma^{-1}
\end{pmatrix}
\begin{pmatrix}
z_{11} \\ \vdots \\ z_{1k} \\ z_{21} \\ \vdots \\ z_{2k} \\ \vdots \\z_{n1} \\ \vdots \\ z_{nk}
\end{pmatrix}  = 
\mathrm{vec}_r(z) ^{\top} (W \otimes \Sigma^{-1}) \mathrm{vec}_r(z)
$$
Then, consider:
$$
 \sum_{i=1}^n \sum_{k=1}^n w_{ik} z^{\top}_i \Sigma^{-1} z_k = 
\begin{pmatrix}
z_{11} \\ \vdots \\ z_{1k} \\ z_{21} \\ \vdots \\ z_{2k} \\ \vdots \\z_{n1} \\ \vdots \\ z_{nk}
\end{pmatrix}^{\top}
\begin{pmatrix}
0 & w_{12} \Sigma^{-1} & \ldots & w_{1n} \Sigma^{-1}\\
w_{21} \Sigma^{-1} & 0 & \ldots & w_{2n} \Sigma^{-1}\\
\vdots & \vdots & \ddots & \vdots \\
w_{n1} \Sigma^{-1} & w_{n2} \Sigma^{-1} & \ldots & 0
\end{pmatrix}
\begin{pmatrix}
z_{11} \\ \vdots \\ z_{1k} \\ z_{21} \\ \vdots \\ z_{2k} \\ \vdots \\z_{n1} \\ \vdots \\ z_{nk}
\end{pmatrix}  = 
(\mathrm{vec}_r z) ^{\top} (D \otimes \Sigma^{-1}) \mathrm{vec}_r z
$$

This proves the joint distribution of $z$ is \citep{Mardia, Banerjee, Gelfand}:
%
\begin{equation}
\pi(z  | \Sigma^{-1}) \propto -\frac{1}{2} (\mathrm{vec}_r\,z)^{\top} (\boldsymbol{L} \otimes \Sigma^{-1})\, \mathrm{vec}_rz
\label{eq:rowwise}
\end{equation}
%
In practical applications (see chapter \ref{chapter:INLA}) it is convenient to rearrange $z$ column-wise through the $\mathrm{vec}$ operator \cite{INLAMSM}, such that $\mathrm{vec}\,z = (z_{11},\, \ldots, \,z_{n1},\, z_{12},\, \ldots,\, z_{n2},\, \ldots ,\, z_{1k}, \,\ldots, \,z_{nk} )^{\top}$. To do so, we are actually using an orthogonal permutation matrix $\mathcal{P}$, such that 
$$\mathrm{vec}\,z = \mathcal{P} \mathrm{vec}_r z $$

To define $\mathcal{P}$, consider the $i$-th basis vector of $\mathbb{R}^p$, $e_i$, the $i$-th row of $I_n$, whose $i$-th element is $1$ and all other elements are zero, $\forall i \in [1,n]$. We then have:
$$
\mathcal{P}  := \begin{pmatrix} 
I_n \otimes e_1^{\top} \\ I_n \otimes e_2^{\top} \\ \vdots \\ I_n \otimes e_k^{\top}
\end{pmatrix}
$$
%
...
$$
\mathcal{P} \left( \boldsymbol{L} \otimes \Sigma^{-1} \right) \mathcal{P}^{\top}= \Sigma^{-1} \otimes \boldsymbol{L}
$$
...
%
\begin{equation}
z|\Sigma \sim \mathcal{N}_{kn} \left( 0, \Sigma^{-1} \otimes \boldsymbol{L}^{+}\right)
\label{eq:icarjoint}
\end{equation}
%
...
$$
\bar{\sigma}^{2} = \prod_{i=1}^{n} \sigma_{ii}^{2/n}
$$
...
The ICAR model has a relatively small number of parameters, i.e. $p(p+1)/2$, corresponding to the entries of the precision matrix, and is easy to interpret. However, it can be easily seen that it is not a flexible model, in that it assumes a spatial autocorrelation parameter equal to $1$. It can be thought of as the spatial equivalent of $RW(1)$ processes in time series analysis: it is suited to detect strong spatial structures, but is misspecified when spatial trends are not particularly smooth. In such cases, the possible outcomes are two: either the model fails at capturing random noise in data, or it does adapt to data but at the price of a higher complexity (overfitting). For this reason, some extension have been developed in literature to account for noise. Here we introduce three popular ones, the so-called Proper CAR \citep{PCAR_Gelfand}, the Leroux proper CAR \citep{Leroux}, and the Besag-York-Mollié \citep[BYM][]{BYM, BYM2} convolution model. 

\subsection{The Proper CAR} \label{subsection:pcar}

Thinking analogously as time series analysts, an intuitive generalisation of the ICAR would be to introduce a parameter, say $ \alpha \in [0, 1]$, controlling for spatial autocorrelation \citep[][chapter 10]{Banerjee}. This would yield the conditional prior distribution:
%
$$
z_i | z_{-i}, \Sigma \sim \mathcal{N}_{pn} \left(\sum_{j=1}^{n} \frac{w_{ij}}{d_i}\alpha z_{j}, \frac{1}{d_i} \Sigma \right)
$$
%
Please notice that $\alpha$ is a scalar here. With calculations like those in section \ref{section:icar}, the joint prior becomes:
%
\begin{equation}
z|\Sigma, \alpha \sim \mathcal{N} \left( 0, \Sigma \otimes (\boldsymbol{D} - \alpha \boldsymbol{W})^{-1}\right)
 \label{eq:pcar}
\end{equation}
%
On a side note, this is a proper distribution in that for $\alpha < 1$ the precision matrix is full-rank \footnote{For this reason, this model has been known in the literature as the 'proper' CAR. Properness is far from an exclusive feature of this model, and is not even limited to the models listed in this section, yet this naming has become a convention, and we respect this convention for notation brevity.}. One advantage of this model is the ease in interpretation, as $\alpha$ quantifies the extent to which the value of $z_j$ depends on the neighbouring sites. Clearly, the limit case for $\alpha = 1$ is the ICAR, while the limit case for $\alpha = 0$ is an independent but \textit{not} identically distributed model, in which the variance is proportional to the inverse degree matrix $\boldsymbol{D}^{-1}$, hence a site with a higher number of neighbours will have a smaller prior conditional variance.
...
%
$$
z_i | z_{-i}, \Sigma \sim \mathcal{N}_{p} \left(\sum_{j=1}^{n} \frac{ \lambda w_{ij}}{1 - \lambda  + \lambda d_i} z_{j}, \frac{1}{1 - \lambda + \lambda d_i} \Sigma\right)
$$
%
...
%
\begin{equation}
z|\Sigma, \lambda \sim \mathcal{N}_{pn} \left( 0,    \Sigma
  \otimes [\lambda \boldsymbol{L} + (1-\lambda)\boldsymbol{I}_n ]^{-1}\right)
 \label{eq:lcar}
\end{equation}
%
...
$U_j \sim \mathcal{N}_n(0, \boldsymbol{L_\mathrm{s}}^{+}) \quad \forall j \in [1,p]$
and $\mathrm{vec}\boldsymbol{U} \sim \mathcal{N}_{pn}(0, I_p \otimes \boldsymbol{L}^{+})$.
The same assumption holds for the IID component, i.e. $\mathrm{vec}\boldsymbol{V} \sim \mathcal{N}_{pn}(0, I_p \otimes I_n)$. the joint distribution of $z$ is then:
%
\begin{equation}
...
For all the autoregressive models discussed above, we observed that the precision (or the covariance) matrix can be conveniently factorised as the Kronecker product of a precision (or scale, respectively) parameter and a structure matrix, which is deterministic for the ICAR and unknown but independent on the precision parameter for the other models; in this latter case, the structure matrix only depends on one scalar-valued parameter (the AR parameter for the PCAR, the precision mixing for the LCAR and the variance mixing for the BYM). The hypothesis of a constant hyperparameter for \textit{all} the spatial fields can be, however, strong. A more flexible class of models, introduced by \cite{Mmodels}, leverage on the decomposition of the scale parameter $\Sigma$ into $\boldsymbol{M}^ \top \boldsymbol{M}$, where $\boldsymbol{M}$ is a generic $k \times p$ full-rank matrix for any $k$; these models are known in literature as M-models. M-models generalise proper autoregressives by relaxing the hypothesis of a scalar-value hyperparameter controlling the strength of spatial association and allow for $k$ different hyperparameters.

The core idea is defining $\boldsymbol{z} := \boldsymbol{z}^{(0)} \boldsymbol{M}$, where $\boldsymbol{z}^{(0)} = (z_1^{(0)}, z_2^{(0)} \dots z_k^{(0)})$ and the $z_1^{(0)} \dots z_k^{(0)}$ fields are independent, each one following a prior of the form: $z_j^{(0)} \sim \mathcal{N}(0, \Omega_j) \quad \forall j \in[1,k]$ where $\Omega_j$ is an inverse structure matrix; taking the PCAR model as an example we have $\Omega_j = (\boldsymbol{D} - \alpha_j \boldsymbol{W})^{-1}$.

Even though the $\boldsymbol{M}$ matrix does not have to be squared, if the number of variables is not prohibitive, models with squared $\boldsymbol{M}$, i.e. $k=p$, are easier to interpret and allow for exactly one spatial association parameter for each variable. 

Take the PCAR model as an example. With $k=p$, equation \ref{eq:pcar} can be generalised as $\mathrm{Var}[z | \Sigma, A] = \Sigma \otimes(I_p \otimes \boldsymbol{D} - A \otimes \boldsymbol{W})^{-1}$, where $A$ is diagonal, and in the block-factorisable case seen in subsection \ref{subsection:pcar} it was $A = \alpha I_p$. When $A = \mathrm{diag}(\alpha_1, \alpha_2, \dots, \alpha_p)$ the variance becomes
%
\begin{equation}
\text{PCAR:} \quad \quad 
\mathrm{Var}[ \mathrm{vec}\,z | \Sigma, A] = (\boldsymbol{M^\top \otimes I_n})
(I_p \otimes \boldsymbol{D} - A \otimes \boldsymbol{W})^{-1} (\boldsymbol{M} \otimes I_n)
\end{equation}
%
...
\begin{equation}
\text{LCAR:} \quad \quad 
\mathrm{Var}[\mathrm{vec} \,z | \Sigma, \Lambda] = (\boldsymbol{M^\top \otimes I_n})
 [\Lambda \otimes \boldsymbol{L} + (\boldsymbol{I}_p - \Lambda) \otimes \boldsymbol{I}_n]^{-1} (\boldsymbol{M} \otimes I_n)
\end{equation}
%
Regarding the BYM, throughout this thesis we will rely on the simplifying hypothesis that:
%
\begin{equation}
z = \boldsymbol{U} \Phi^\frac{1}{2} \boldsymbol{M} + 
\boldsymbol{V} (I_p - \Phi)^\frac{1}{2} \boldsymbol{M}
...
\begin{equation}
    \text{BYM:} \quad \quad
    \mathrm{Var}[\mathrm{vec} \, z | \Sigma, \Phi] = (\boldsymbol{M^\top \otimes I_n})
 \  [ \Phi \otimes \boldsymbol{L} + (\boldsymbol{I}_p - \Phi) \otimes \boldsymbol{I}_n]
 (\boldsymbol{M} \otimes I_n)
 \label{eq:mmbym}
\end{equation}
%
...
To gain more insight on restricted regression, let us briefly outline a general latent Gaussian model, which will be useful to explain the current section, and consider:
$$
\left\{ \begin{array}{ll} 
...
\end{array} \right. 
$$
where $y$ denotes the respondse variable consisting of $N$ observations; $\eta$ is the linear predictor linked with a function $g$ to the expected value of $y$; $\vartheta$ is a generic vector of latent variables of interest; $\sigma_{\varepsilon}^2 := \mathrm{Var}[\varepsilon]$ is the variance of the IID Gaussian error $\varepsilon$, $\boldsymbol{A}$ is a known design matrix; $\Psi$ is the array of hyperparameters, and $\boldsymbol{Q_{\Psi}}$ is the precision matrix of $\vartheta$. %In the case of the multivariate ICAR, as seen in Section \ref{section:icar}, the precision is $\boldsymbol{Q}_{\Psi} = \Sigma^{-1} \otimes \boldsymbol{L}$, where $\Sigma$ is the scale parameter and $\boldsymbol{L}$ is the graph Laplacian matrix. For the univariate case, the precision parameter clearly reduces to a positive scalar.

The linear predictor, to our aims, can be defined as:
$$  
\boldsymbol{A} := \left(X \quad \xi \right) \quad \text{and} \quad
\vartheta := \begin{pmatrix} \beta \\ \boldsymbol{z} \end{pmatrix}
$$
where $\beta$ is the array of regression coefficients of length $p$, $z$ is the array of spatial latent effects\footnote{Labelling $\beta$ as "fixed" and $z$ as "random" effects is quite frequent in hierarchical regression applications, but due to the polysemic nature of these terms we will tend to avoid them. In a strictly probability perspective, there is no conceptual distinction between them: they are both unknown random variables entering the linear predictor through a known design matrix (either $X$ or $\xi$), are typically assumed to be Gaussian \textit{a priori} and their posterior distribution is the primary aim of statistical inference. We keep them separated due to how they are interpreted: $\beta$ represents the association between a set of known variables ($X$) and $y$, while $z$ is an unobserved process shaping the structure of $y$ itself.} of length $n$, $X$ is a matrix of explanatory variables of size $N \times p$, and $\xi$ is the design matrix of latent effects of size $N \times n$. In most cases, $N=n$, though this assumption is not necessary and is violated in the case study outlined in Chapter \ref{Chapter:Invalsi}.

Let us further assume that $(y_j\perp\!\!\!\perp y_k )|\vartheta, \Psi$ and $(y_j\perp\!\!\!\perp \eta_k )|\vartheta, \Psi$ for any $j,k \in [1, N]$ with $j \neq k$, so each observation $y_j$ depends on $\vartheta$ only through one value $\eta_j$ and the likelihood can be factorised as follows:
$$
\pi(y|\vartheta, \Psi) = \pi(y| \eta, \Psi) = \prod_{j = 1}^N \pi(y_j | \eta_j, \Psi)
$$
...
%in order to map each observed value $y_j$ to one realisation of the random effect $z_i$, i.e. if $y_j$ depends on $z_i$ and therefore $\xi_{ji} = 1$, then  $y_j \perp\!\!\!\perp  z_{-i}$, with $i \in [1, n]$ and $j \in [1, N]$, where each row of $\xi$ has exactly one entry equal to $1$ and all other entries equal to $0$. 
 ...
 For illustration purposes, let us now assume that $y$ is Gaussian and the linear predictor includes a univariate Gaussian latent effect of size $n$ with prior mean $0$ and non-negative defined precision matrix $\boldsymbol{Q}(\Psi) = \tau \boldsymbol{R}$, where $\boldsymbol{R}$ is a semidefinite positive structure matrix and $\tau$ is a global precision parameter; for the ICAR case, clearly $\boldsymbol{R} \equiv \boldsymbol{L}$. In such Gaussian regression framework, the model becomes:
%
\begin{equation}
%\begin{aligned}
\left\{ \begin{array}{ll} 
    y = X \beta + \xi z + \varepsilon \\
    z \sim \mathcal{N} (0, \tau^{-1} \boldsymbol{R}^{-1}) \\
    \beta \sim \mathcal{N}(0, M)\\
    \varepsilon \sim \mathcal{N}(0, \tau_{\varepsilon}^{-1}I_n)
    \end{array} \right.
%\end{aligned}
\end{equation}
%
The covariate matrix $X$ has size $n \times p$; $\xi$ is the model matrix for the spatial effects (usually the $n \times n$ identity matrix), $\tau_\varepsilon$ is the error precision parameter and $M$ is an arbitrarily large scalar.

This model can be parametrised in a more general way \citep[see e.g.][]{DIC}:
%
\begin{equation}
\begin{aligned}
y \sim \mathcal{N}(A_1 \vartheta_1,\tau_{\varepsilon}^{-1} I_n ) \\
\vartheta_1 \sim \mathcal{N}(A_2  \vartheta_2,\tau_{\varepsilon}^{-1} C)
\end{aligned}
\label{eq:LGM_y_theta}
\end{equation}
%
Where  $A_1 = (X \, \xi)$, and $\vartheta_1 = (\beta^{\top} z^{\top})^{\top}$ is the vector of parameters. The term $A_2 \vartheta_2$ has by construction both mean and variance zero. Moreover,
$$ 
C^{-1} = \tfrac{\tau}{\tau_{\varepsilon}} \begin{pmatrix}
 \frac{1}{\tau M} \, I_p & 0_{p \times n} \\ 0_{n \times p} & \boldsymbol{R}
\end{pmatrix}
$$

Since the error term $\varepsilon$ and the latent effects $z$ have different precision parameters, the matrix $C^{-1}$ cannot be considered known. To see this, consider the prior precision of the random effects defined as $\tau \boldsymbol{R}$; $\tau_{\varepsilon} C^{-1}$ must thus be structured as $\begin{pmatrix} \frac{1}{M} \, I_p & 0_{p \times n} \\ 0_{n \times p} & \tau \boldsymbol{R} \end{pmatrix}$ where $M$ is the known variance parameter of $\beta$, but $\tau$ is unknown and isolating $\tau$ to express $\tau_{\varepsilon} C^{-1} = \tau \begin{pmatrix}
 \frac{1}{\tau M} \, I_p & 0_{p \times n} \\ 0_{n \times p} & \boldsymbol{R}
...
\begin{pmatrix}
 ...
 \xi ^{\top} X & \xi^{\top} \xi + \bar{r} \boldsymbol{R}
\end{pmatrix}^{-1} = \\ = 
\begin{pmatrix}
G^{-1} + G^{-1} X^{\top} \xi S_{11}^{-1} \xi ^{\top} XG^{-1} & 
-G^{-1} X^{\top} \xi S_{11}^{-1}\\
-  S_{11}^{-1} \xi^{\top} X G^{-1} & 
 S_{11}^{-1}
\end{pmatrix}
...
\end{equation}
%\begin{pmatrix}
%G^{-1} + G^{-1} X^{\top} \xi(\xi ^{\top} \xi + \bar{r} \boldsymbol{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1} \xi ^{\top} XG^{-1} %& 
%-G^{-1} X^{\top} \xi(\xi ^{\top} \xi + \bar{r} \boldsymbol{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1}\\
%-  (\xi ^{\top} \xi + \bar{r} \boldsymbol{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1} \xi^{\top} X G^{-1} & 
% (\xi ^{\top} \xi + \bar{r} \boldsymbol{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1}
%\end{pmatrix}

Where $S_{11} := (\xi ^{\top} \xi + \bar{r} \boldsymbol{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)$ is the Schur complement of the upper-left block in $\left(A_1^\top A_1 + C^{-1}\right)^{-1}$, $\bar{r} := \dfrac{\tau}{\tau_{\varepsilon}}$ and $G := (X^{\top}X + r_{\varepsilon}I_p)$, where $r_{\varepsilon} := \dfrac{1}{\tau_{\varepsilon}M} = \dfrac{\sigma_{\varepsilon}^{2}}{M}$.
It can thus be shown that:
\begin{equation}
\begin{aligned}
E[\beta | y, \tau_{\varepsilon}, \bar{r}] = G^{-1} \lbrace X^{\top} 
+ X^{\top} \xi S_{11}  ^{-1} \xi^{\top} X G^{-1} X^{\top} 
- X^{\top} \xi S_{11} ^{-1} \xi^{\top}
\rbrace \, y
\end{aligned}
\label{eq:expbeta}
\end{equation}
And
\begin{equation}
E[z | y, \tau_{\varepsilon}, \bar{r}] =  S_{11}  ^{-1}
(\xi^{\top} - \xi^{\top} X G^{-1} X^{\top}) \, y
\label{eq:expzeta}
\end{equation}

Equivalently, if for brevity we define $\Gamma = \xi ^{\top} \xi + \bar{r} \boldsymbol{R} - \xi^{\top} X G^{-1} X^{\top} \xi$ this means that the hat matrix equals:
\begin{equation}
\begin{aligned}
H_{\tau_{\varepsilon}, \bar{r}} = XG^{-1}X^{\top} + \xi \Gamma^{-1} \xi^{\top} -
XG^{-1}X^{\top} \xi \Gamma^{-1} \xi^{\top} -  \xi \Gamma^{-1} \xi^{\top}XG^{-1}X^{\top} +\\
+XG^{-1}X^{\top} \xi \Gamma^{-1} \xi^{\top} XG^{-1}X^{\top}
\end{aligned}
\label{eq:projmat}
\end{equation}
...
%
\begin{equation}
I(\boldsymbol{W}) = \frac{n}{\boldsymbol{1^\top W 1}} \frac{z^\top \mathcal{C} \boldsymbol{W} \mathcal{C}z}{z^\top \mathcal{C}z}
\label{eq:I_moran}
\end{equation}
%
where $\mathcal{C}$ is the idempotent centering matrix $\mathcal{C}:=\boldsymbol{I}_n - \frac{1}{n}\boldsymbol{11}^\top$, hence  $\mathcal{C}z = z - \bar{z}$. 
...
As mentioned in section \ref{subsection:Mmodels}, we use a unique scale parameter $\Sigma$ for the two components, and define $\boldsymbol{M}$ as a generic full-rank matrix such that $\boldsymbol{M}^{\top}\boldsymbol{M} = \Sigma$. For convenience, we also define the precision parameter $\boldsymbol{T} =: \Sigma^{-1}$.
...
%
$$
\mathbb{E} [\mathrm{vec}\, \boldsymbol{z} \mid U, \Phi , \Sigma] = \mathrm{vec}(\boldsymbol{U} \Phi^{\frac{1}{2}} \boldsymbol{M}) =
...
%
$$
\mathrm{Var} [\mathrm{vec}\, \boldsymbol{z} \mid \boldsymbol{U}, \Phi , \Sigma] =
...
$$
%
The distribution of $\boldsymbol{Y} \mid \boldsymbol{U}, \Sigma, \Phi $ then reads:
\begin{align*}
-2 \ln \pi \left(\mathrm{vec} \, \boldsymbol{Z} \mid \boldsymbol{U}, \Sigma, \phi \right) =\\
 %
...
And
\begin{align*}
-2 \ln \pi \left(\mathrm{vec} \,\boldsymbol{z}, \mathrm{vec}\,\boldsymbol{U} \mid \Sigma, \phi \right)  
= C + (\mathrm{vec}\,\boldsymbol{z})^{\top}
\left(\boldsymbol{q}_{11} \otimes \boldsymbol{I}_n \right) 
\mathrm{vec}\,\boldsymbol{z} +\\ 
%
...
Nevertheless, an alternative approach has emerged in the last twenty years, the Integrated Nested Laplace Approximation . The core idea behind INLA is to directly apply a deterministic approximation to the posteriors of interest, and dates back to the proposal by \cite{tierney1986accurate} to approximate the first and second moments of the parameters. The Laplace approximation leverages on the convergence of the full posterior to a Gaussian distribution when parameters are Gaussian \textit{a priori}, the approximation error being in $\mathcal{O}(n^{-1})$ where $n$ is the length of the target variable. 
...
\right)
$$
 Where $\boldsymbol{\Sigma_{BB}} : =\boldsymbol{\Sigma_{AA}} - \boldsymbol{\Sigma_{AB}} \boldsymbol{\Sigma^{-1}_{BB}} \boldsymbol{\Sigma_{BA}}$ is the Schur complement of $\boldsymbol{\Sigma_{BB}}$ in $\boldsymbol{\Sigma}$
\citep[][paragraph 0.7.3]{HornJohnson}. Then the precision reads as:
\begin{equation}
	\boldsymbol{Q} = 
	\left( \begin{array}{ll} 
...
$$
%
As for $\boldsymbol{S_{BB}}$, we may define: $\boldsymbol{S_{AA}} : =\boldsymbol{\Sigma_{BB}} - \boldsymbol{\Sigma_{BA}} \boldsymbol{\Sigma^{-1}_{AA}} \boldsymbol{\Sigma_{AB}}$ and notice that
$$
...
\left(\boldsymbol{\Sigma_{BB}} - \boldsymbol{\Sigma_{BA}} \boldsymbol{\Sigma^{-1}_{AA}} \boldsymbol{\Sigma_{AB}} \right) =
$$
$$
= \boldsymbol{I} 
- \boldsymbol{\Sigma_{BB}^{-1}} \boldsymbol{\Sigma_{BA}} \boldsymbol{\Sigma_{AA}^{-1}} \boldsymbol{\Sigma_{AB}}
+ \boldsymbol{\Sigma_{BB}^{-1}} \boldsymbol{\Sigma_{BA}} \boldsymbol{S_{BB}^{-1}}
  \boldsymbol{\Sigma_{AB}} +$$
$$
- \boldsymbol{\Sigma_{BB}^{-1}} \boldsymbol{\Sigma_{BA}}      \boldsymbol{S_{BB}^{-1}}
  \boldsymbol{\Sigma_{AB}}      \boldsymbol{\Sigma_{BB}^{-1}} \boldsymbol{\Sigma_{BA}}
  \boldsymbol{\Sigma^{-1}_{AA}} \boldsymbol{\Sigma_{AB}} = 
$$
$$
= \boldsymbol{I} - \boldsymbol{\Sigma_{BB}^{-1}} \boldsymbol{\Sigma_{BA}} \,
\left( \boldsymbol{S_{BB}^{-1}}  - \boldsymbol{S_{BB}^{-1}}  \boldsymbol{\Sigma_{AB}}      \boldsymbol{\Sigma_{BB}^{-1}} \boldsymbol{\Sigma_{BA}}
\boldsymbol{\Sigma^{-1}_{AA}} - \boldsymbol{\Sigma^{-1}_{AA}} \right) \,
\boldsymbol{\Sigma_{AB}} =
$$ 
$$
\boldsymbol{I} - \boldsymbol{\Sigma_{BB}^{-1}} \boldsymbol{\Sigma_{BA}} \,
\left[
\boldsymbol{S_{BB}^{-1}} - \boldsymbol{S_{BB}^{-1}} \left(\boldsymbol{\Sigma_{AA}} - \boldsymbol{S_{BB}} 
 \right)\boldsymbol{\Sigma^{-1}_{AA}} - \boldsymbol{\Sigma^{-1}_{AA}}
\right] \, \boldsymbol{\Sigma_{AB}} = \boldsymbol{I}
$$
...
The ideal yet not exclusive field of application of the INLA are hierarchical generalised linear regression models of this form:
%
$$
\left\{ \begin{array}{ll} 
...
What researchers are typically interested in are the posterior marginal distributions $\pi(\vartheta_i | y)$ and $\pi(\Psi_j | y)$  $\forall i \in [1, n+p]$ and $j \in [1, \mathrm{card}(\Psi)]$, which are obtained by solving the integrals:
 \begin{equation}
 \label{eq:marginals}
 \begin{aligned}
 %\left\{ \begin{array}{ll}
 ...
 \end{aligned}
 \end{equation}
Integration is feasible provided that $\Psi$ has a small size. However, this operation can hardly be completed in closed form. This calls for the need of approximations, which we are going to discuss in the following.
...
{\pi(\vartheta | y, \Psi)\pi(y)} 
 ...
 $$
 and the integral can only be solved in closed formula for conjugate priors on $\vartheta$. The intuition behind the INLA is, therefore, to replace the integrand function with its Gaussian approximation, which we denote as $\pi_G(\vartheta, y \mid \Psi)$
\begin{equation}
...
$$
\nabla \gamma(\vartheta) = \alpha(\Psi) \left( A^\top y - A^\top \Delta_1 \right) - \frac{1}{2}\boldsymbol{Q}_\Psi \vartheta
$$
...
$$
where $\boldsymbol{\Delta_2}= \mathrm{diag}{\left(\frac{d^2b}{d\eta_1^2},\frac{d^2b}{d\eta_2^2}, \cdots, \frac{d^2b}{d\eta_N^2} \right)}$. The Gaussian model in section \ref{subsection:LGM} would have $\Delta_1 \equiv \eta$ and $\boldsymbol{\Delta_2} \equiv \boldsymbol{I}$ (cfr. section \ref{subsection:LGM}), and $\alpha(\Psi) = \sigma_{\varepsilon}^{-2}$ a Poisson model would have $h(\eta) = e^{\eta}$ and $\alpha(\Psi) = 1$, and so on.
...
$$
\pi(\Psi |y) \approx \pi_{LA}(\Psi | y) \propto  \left. \frac{\pi(y | \eta, \Psi) \pi(\vartheta | \Psi) \pi(\Psi)}{\pi_G(\vartheta | y, \Psi)} 
\right \vert_{\vartheta = \vartheta_0(\Psi, y)}
$$
we have seen in the example \ref{ex:GLM_INLA} that computing such Gaussian approximation and locating the relevant mode is a simple task \textit{per se}. The Laplace approximation on hyperparameter posterior follows straightforwardly.


\subsection{Approximating joint Gaussian field Posterior}\label{par:inla_theta}

To approximate the marginals $\pi(\vartheta_i \mid y, \Psi) \forall i$, several solutions are possible. 

\subsubsection{The original framework} \label{subsection:inla_original}
\cite{INLA}, in their seminal work, proposed a set of three alternative strategies, following a trade-off in accuracy and computational cost.
...
The third approach is known as "simplified Laplace" approximation, representing a compromise between the former two approaches in terms of accuracy and computational cost; it basically consists in truncating the Taylor approximation of $\gamma(\vartheta)$ at the third order term, while still locating the approximation at $\vartheta_0(y, \Psi)$. This allows to fit a Skew-Normal density to $\gamma(\vartheta)$. 

For more details on how the parameters vector is defined, see \cite{INLA2017}. 

\subsubsection{The Variational Bayes method}

A recent strand of literature introduced a different approach to approximating the joint conditional posterior of the latent Gaussian field. While the former methods consisted in three proposed functional forms of this approximation, the new strategy takes a further step in the field of optimisation. In this context, optimisation must be read as proposing an approximation as close as possible to the true joint posterior, i.e. $\pi(\vartheta \mid y, \Psi)$. To this aim, the simplest Gaussian approximation $\pi_G(\theta \mid y, \Psi)$ remains \textit{per se} a rigorous proposal as a functional form, while the element on which attention is drawn are now its moments. 

Intuitively, approximating the true but unknown density $\pi(\vartheta \mid y, \Psi)$ with a known density $\pi_G(\vartheta \mid y, \Psi)$ implies a loss in information. As argued by \cite{Zellner1988}, statistical inference consists in its very essence in the processing of two prior assumptions, the likelihood and parameter prior density, into two outputs, the marginal likelihood and the posterior distribution of parameters. The Bayes rule itself implies that no loss of information occurs when the output quantities are computed exactly. But prior conjugacy is a fortunate case, and if it held in general, humanity would have never bothered inventing the science of approximate inference, and this whole chapter would not even have any reason to be at all. 
...
%
$$
\pi_\mathrm{VB}(\vartheta \mid y, \Psi) := \varphi\left(\vartheta \mid \vartheta_1, \left(-\boldsymbol{H_\gamma}\right)^{-1}\right)
$$
%
...
$$
\vartheta_1:  \vartheta_0(y,\Psi) + \Sigma_I \lambda
$$
%
...
KLD\left( \pi_{VB}(\vartheta | y, \Psi) \mid \mid \pi(\vartheta | \Psi)\right) + \\
-\int_{-\infty}^{+\infty}  \sum_{i=1}^{N} \ln\pi(y_i | \eta_i, \Psi) \pi_{VB}(\vartheta \mid y, \Psi)\, d \vartheta \Bigg{\rbrace}
\label{VBINLA}
\end{aligned}
\end{equation}
...
%This latter approach consists fitting the Gaussian approximation, then correcting $\vartheta_0$ (as in eq. \ref{eq:apprG}) by an additive vector, say $\lambda$, whose entries are nonzero only for a subset $I$ drawn from the total set of indices of $\vartheta$, namely $I \subset \left\{ 1, 2, ... n\right \}$:
%$$
%\pi_{VB}(\vartheta | y, \Psi) \propto e^{\displaystyle{
% - \frac{1}{2}\left[\vartheta - \vartheta_0(y,\Psi)- \Sigma_I \lambda\right]^\top
% \boldsymbol{H_\gamma}(\theta_0(y,\Psi))
% \left[\vartheta - \vartheta_0 (y, \Psi)- \Sigma_I \lambda\right]
%}}
%$$
This methodology appears promising, as it attempts to face the essential issue of approximate inference, i.e. the very fact that the output information is an approximate function. However, it bears two limits. First, only the mean of the posterior is corrected. Second, the posterior functional form is assumed rigidly. To this aim, some refinements are being carried forward \citep{dutta2025scalable}. On the first issue, a Variational Bayes additive correction has been proposed for the precision matrix, and it is seen to improve the accuracy of the posterior approximation for some selected likelihoods.

\subsection{Numerical integration} \label{par:inla_int}

Once the integrand functions in \ref{eq:marginals} are defined, integration is carried out numerically. This requires to compute $\tilde{\pi}(\Psi \mid y)$ across a given set of points, which in this case is a multidimensional grid. In particular, this grid exploration will not regard $\Psi$ itself, but a transformation defined over $\mathbb{R}$, whereas many hyperparameters usually range on a subset of $\mathbb{R}$ (think of variance and precision parameters which are defined on the nonnegative axis, or correlations which range -1 to 1). 

The fundamental assumption to make is that $\tilde{\pi}(\Psi \mid y)$ is unimodal. In a first stance, unimodality implies that the hyperparameter posterior tends to zero as $\Psi$ departs from its mode. Thus, numerical integration can be carried out within a neighbourhood of hyperparameter mode and not on its entire domain. In a second stance, under this assumption only one value for the local maximum of the hyperparameter posterior needs being found. Locating the mode of $\tilde{\pi}(\Psi \mid y)$ is the most computationally demanding step of the INLA procedure and, at least in my personal experience, the most delicate by far.

\cite{INLA} proposed a quasi-Newthon methodology to locate the mode. As we shall see in the next paragraph, this approach is still valid today in practical implementations. Then, starting from the mode, the authors propose some alternative integration schemes.
The cheapest solution possible in theory relies on the so-called empirical Bayes methods, and consists in assuming that hyperparameter posterior is sharply peaked around its mode, hence the integrals in equation \ref{eq:marginals} are replaced by the integrand functions evaluated at the mode of $\Psi$. This solution rules out all the uncertainty on the joint distribution of hyperparameters.
...
\textcolor{red}{Ampliare: descrivere la sequenza di operazioni nella function call \texttt{inla(...)}}
...
Mapping the Matérn field onto a GMRF shrinks the computational cost to $\boldsymbol{\theta}(N^{3/2})$. \textcolor{red}{TBD: AGGIUNGERE FONTE O DIMOSTRARE}
...
To ease the interpretation of $T$ as the precision parameter of $z$, it is possible to cleanse it from the effect of the neighbourhood structure by defining a scaled version of $\boldsymbol{L}$ \cite{Sorbye} and reparametrising the precision of $z$ accordingly. The scaling procedure is described in subsection \ref{subsection:scaling}.
Since $\mathcal{G}$ is disconnected, each component-specific block of $\boldsymbol{L}$ is multiplied by the relevant typical variance, namely the geometric mean of the diagonal of the corresponding block of its pseudoinverse, following the methodology proposed by \cite{Freni}. It is therefore possible to define a precision parameter $T_\mathrm{scaled}$ which is not confounded with graph-induced effects. The scaled precision is assigned a Wishart prior \citep{Gelman} with $2k+1$ degrees of freedom and scale parameter equal to the identity matrix, i.e. $T_\mathrm{scaled} \sim \mathrm{Wishart}_{k}( I_k, 2k+1)$, with $k=2$ \citep{INLAMSM}.
