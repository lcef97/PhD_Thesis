...
%
Let us introduce the row-wise vectorising operator $\mathrm{vec}_r$, which stacks row-wise the elements of a $n \times p$ matrix into a $pn \times 1$ vector. First consider:
$$
...
%
\begin{equation}
\pi(z  | \Sigma^{-1}) \propto -\frac{1}{2} \mathrm{vec}_r(z) ^{\top} (\mathbf{L} \otimes \Sigma^{-1})\, \mathrm{vec}_r(z)
\label{eq:rowwise}
\end{equation}
%
...
$$\mathrm{vec}\,z = \mathcal{P} \times \mathrm{vec}_r z $$
...
%
Considering that$
\mathrm{Var}[\mathrm{vec}\, z] = \mathcal{P} \times \mathrm{Var}[\mathrm{vec}_rz] \times \mathcal{P}^{\top}$, hence $\mathrm{Prec}[\mathrm{vec}\, z] = \mathcal{P} \times \mathrm{Prec}[\mathrm{vec}_r z] \times \mathcal{P}$, it holds that:
$$
...
%
\begin{equation}
z|\Sigma \sim \mathcal{N} \left( 0, \Sigma^{-1} \otimes \mathbf{L}^{+}\right)
\label{eq:icarjoint}
\end{equation}
%
...
We have just seen that under RSR the posterior means of covariate effects approximate those of a non-spatial model, and if $y$ is Gaussian and $\beta$ has a flat prior, equality holds. Interestingly, \cite{DupontArXiv} argue that setting such a constraint would indeed yield a bias in $\mathbb{E}[\beta |y]$ if confounding occurs: intuitively, the expectation would be approximately the same of a model which ignores the existence of the spatial component. In terms of interpretation, this would mean that adding a spatial effect to the model should not significantly alter the estimated regression coefficients.

For this reason, a recent strand of literature criticised RSR, starting from \cite{Khan} who proved analytically how RSR yields to similar coefficient estimates as nonspatial regression (equality holds at least in the case of Normal likelihood and Uniform prior on the coefficients), yet it underestimates coefficient variability, resulting in reduced coverage with respect to nonspatial regression. 

\subsubsection{Filtering based on Moran operator}
Before criticism against RSR, several alternative approaches have being proposed in the last twenty years. One of the earliest ones was introduced by \cite{HughesHaran} and can be thought of as a refinement of RSR. In brief, they noticed the constraints used in RSR are not informative on the spatial structure of data, hence they developed a restriction methodology based on the eigenvectors of what authors define the generalised Moran operator, rather than the projection matrix.

\paragraph{The Moran coefficient} \label{par:I_moran}
To gain some context, let us first recall that the Moran index $I(x)$ referring to a vector $x$ of length $n$ taking values over a graph with neighbourhood matrix $\mathbf{W}$ is given by 
%
\begin{equation}
I(x) = \frac{n}{\mathbf{1^\top W 1}} \frac{x^\top \mathcal{C} \mathbf{W} \mathcal{C}x}{x^\top \mathbf{W}x}
\label{eq:I_moran}
\end{equation}
%
where $\mathcal{C}$ is the idempotent centering matrix $\mathcal{C}:=\mathbf{I}_n - \frac{1}{n}\mathbf{11}^\top$ implying  $\mathcal{C}x = x - \bar{x}$

\paragraph{Smoothing orthogonal to the}
...
This is particularly interesting in that the collinearity between the unmeasured confounder and the explanatory variables is ruled out by means of a spatial filter and not by automatic constraints. As an example, \href{https://github.com/lcef97/PhD_Thesis/blob/main/docs/Media/Moran_Eigenvectors.gif}{in this gif} we show the structure of the 30 eigenvectors associated with the largest eigenvalues of a Moran operator defined on a set of 4 randomly generated Uniform variables over a $30 \times 30$ regular lattice; the neighbourhood matrix in this case follows the rook criterion (neighbours must share an edge).
