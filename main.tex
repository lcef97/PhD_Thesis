...
\end{equation}
Where $C_1$ is an additive constant independent on $z$. Now, let us introduce the row-wise vectorising operator $\mathrm{vec}_r$, which stacks row-wise the elements of a $n \times p$ matrix into a $np \times 1$ vector. First consider:
$$
...
This proves the joint distribution of $z$ is \citep[][this latter case regarding the proper CAR]{Mardia, Banerjee, Gelfand}:
\begin{equation}
...
As it can be seen in the previous equations, the ICAR precision is the product of two terms, the global precision (inverse scale) and a structure matrix. This hinders parameter interpretation at the global level; to start discussing it, let us first consider the case of a connected graph. Denoting with $\mathbf{S} : = \mathbf{L}^{+}$, the marginal variance of a generic $i$-th realisation of the latent effect for the $h$-th variable is: 
$$
...
or typical marginal variance). We are thus able to separate the parameter $\Sigma$ from the effect on variance induced by the graph structure. To do so, the scaled model does not employ $\mathbf{L}$ as the structure matrix, but the scaled matrix $\mathbf{L}_\mathrm{scaled} = \bar{\sigma}^2 \mathbf{L} $. On the other hand, the covariance matrix becomes $\bar{\sigma}^2 \Sigma$, which actually expresses the variability of the process independently of how the graph is structured. 
...
$$
\mathbf{L}_\mathrm{scaled} = \mathcal{P} \left( \bar{\sigma}_1^2 \mathbf{L_1} \oplus \bar{\sigma}_2^2 \mathbf{L_2}
...
%
$$
z_i | z_{-i}, \Sigma \sim \mathcal{N} \left(\sum_{j=1}^{n} \frac{w_{ij}}{d_i}\alpha z_{j}, \frac{1}{d_i} \Sigma \right)
$$
%
...
%
\begin{equation}
z|\Sigma, \lambda \sim \mathcal{N} \left( 0,    \Sigma
...
u \sim \mathcal{N} \left(0, \mathbf{L}_{\mathrm{scaled}}^{+} \right)\\
v \sim \mathcal{N}(0, \mathbf{I}_n)
\end{array} \right.
$$
The interesting feature is that, since the Laplacian matrix is scaled, the two addenda are now defined on the same scale of variability, which implies the mixing parameter can be interpreted as the balancing of variability between spatial structure and noise. In the multivariate case, the joint distribution of $z$ is:
%
\begin{equation}
z|\Sigma, \phi \sim \mathcal{N} \left( 0, 
 \Sigma \otimes (\phi \mathbf{L}_\mathrm{scaled}^+ + (1-\phi) \mathbf{I}_{n})\right)
 \label{eq:pcar}
\end{equation}
%
As can be seen, the precision of the BYM field is a dense matrix. Operations such as inversion, eigendecomposition, or multiplication, would then have cubic computational cost, which in most real-world applications is prohibitive. \cite{BYM2}, therefore, suggest to model $z$ and $u$ jointly, as a process of length $2n$ (or $2nk$ in the multivariate case). The resulting joint process has then a sparse precision matrix, retaining the same rank deficiency as $\mathbf{L}$, which requires the sum-to-zero constraint on the ICAR component $u$. To sum up, the BYM model boasts some desirable features, namely: 
\begin{itemize}
...
\begin{equation}
\text{PCAR:} \quad \quad 
\mathrm{Var}[z | \Sigma, A] = (\mathbf{M^\top \otimes I_n})
...
\text{LCAR:} \quad \quad 
\mathrm{Var}[z | \Sigma, A] = (\mathbf{M^\top \otimes I_n})
\lbrace \Sigma \otimes [\mathbf{L} + (I_p - \Lambda) \otimes \mathbf{I}_n]^{-1} \rbrace (\mathbf{M} \otimes I_n)
\end{equation}
%
...
Here we briefly outline a general latent Gaussian model, which will be useful to explain the current section. Let us consider:
$$
\left\{ \begin{array}{ll} 
...
Equivalently, if for brevity we define $\Gamma = \xi ^{\top} \xi + \bar{r} \mathbf{R} - \xi^{\top} X G^{-1} X^{\top} \xi$ this means that 
\begin{equation}
\begin{aligned}
...
If a flat prior is defined on $\beta$, we have $M \longrightarrow \infty$, with $G \longrightarrow X^{\top}X$. In this case, it is possible to show that the limits for equations \ref{eq:expbeta} and \ref{eq:expzeta} are the results stated by \cite[][page 1199]{RHZ}. Moreover, the hat matrix does not depend on $\tau_{\varepsilon}$ but only on $\bar{r}$.
...
Referring to the flat $\beta$ case, under the restricted model it holds that:
$$
\mathrm{E}[\beta_\mathrm{RSR} | y, z, \tau_{\varepsilon}, \bar{r}] = \beta_\mathrm{OLS} - (X^{\top}X)^{-1} X^{\top}(I - X(X^{\top}X)^{-1}X^{\top})z = \beta_\mathrm{OLS}
$$
Where $\beta_\mathrm{RSR}$ denotes the vector of covariate effects under restricted spatial regression and $\beta_{OLS}$ are covariate effects estimated via the ordinary least squares. This implies $\beta$ is estimated independently of the latent effect, and its expectation equals that of the nonspatial model. Holding a flat prior on $\beta$, referring to equation (6), we see that under the  unrestricted model, with $\xi = I_n$, $\Gamma = \bar{r} \mathbf{R} + P_{\bot}$
\begin{align*}
H_{\tau_{\varepsilon}, \bar{r}} = P + (\bar{r} \mathbf{R} + P_{\bot})^{-1} - P (\bar{r} \mathbf{R} + P_{\bot})^{-1} - (\bar{r} \mathbf{R} + P_{\bot})^{-1} P + P (\bar{r} \mathbf{R} + P_{\bot})^{-1}P = \\
= P + P_{\bot} (\bar{r} \mathbf{R} + P_{\bot})^{-1} P_{\bot}
\end{align*}

Under the restricted model, i.e. using the spatial effect $\zeta = E^{\top}z \sim \mathcal{N}(0, E^{\top}QE)$ we have $\Gamma = \bar{r} E^{\top}QE + I_{n-p}$, hence 
$$
...
RSR can be extended to the multilevel case \cite{Nobre}, in which e.g. spatial effects are defined at a higher scale than observations, as in Chapter \ref{chapter:theory}.


Now, as it has been seen, under RSR the posterior means of covariate effects approximate those of a nonspatial model, and if $y$ is Gaussian and $\beta$ has a flat prior, equality holds. Interestingly, \cite{DupontArXiv} argue that setting such a constraint would indeed yield a bias in $\mathbb{E}[\beta |y]$ if confounding occurs: intuitively, the expectation would be approximately the same of a model which ignores the existence of the spatial component. In terms of interpretation, this would mean that adding a spatial effect to the model should not significantly alter the estimated regression coefficients.
...
First let us consider the multivariate ICAR component. With no loss of generality, we define $\mathbf{U} = (U_1, U_2, \dots U_p)$. We suppose all its univariate components to have the same prior distribution, i.e. 
$$U_j \sim N_n(0, \mathbf{L}^{+}) \quad \forall j \in [1,p]$$
and assume independence among them, such that $\mathrm{vec}(\mathbf{U}) \sim \mathcal{N}_{np}(0, I_p \otimes \mathbf{L}^{+})$.
The same assumption is made on the IID component, i.e. $\mathrm{vec}(\mathbf{V}) \sim \mathcal{N}_{np}(0, I_p \otimes I_n)$.

As mentioned in section \ref{subsection:Mmodels}, we use a unique scale parameter $\Sigma$, and define $\mathbf{M}$ as a generic full-rank matrix such that $\mathbf{M}^{\top}\mathbf{M} = \Sigma$. For convenience, we also define the precision parameter $\mathbf{T} =: \Sigma^{-1}$.

Here, $\mathbf{M}$ does not have to be the Cholesky factor of $\Sigma$; in fact, a rather convenient yet not unique definition is $\mathbf{M} = D^{\frac{1}{2}}\mathbf{E}^{\top}$, where $D$ is the diagonal matrix of the eigenvalues of $\Sigma$ and $\mathbf{E}$ are the eigenvectors of $\Sigma$. Consider the model in equation \ref{eq:mmbym}, it holds that
$$
\mathrm{Var}\left[ \mathrm{vec} \mathbf{z} \mid \Sigma, \Phi \right] = 
\left( \mathbf{M}^\top \otimes \mathbf{I}_n\right) 
\mathrm{diag}\left(\Omega_1,\ldots,\Omega_p\right)
\left( \mathbf{M} \otimes \mathbf{I}_n \right)
$$
Where each diagonal block $\Omega_j$ is the variance of $U_j \Phi + V_j \bar{\Phi}$, i.e. $\Omega_j = \phi_j \mathbf{L}^+ + (1-\phi_j) \mathbf{I}_n$. 
Keeping up with the approach of \cite{BYM2}, and defining for the sake of brevity $\bar{\Phi} := \mathbf{I}_p - \Phi$, we know that:
...
\left[(\bar{\Phi}^{\frac{1}{2}} \mathbf{M}) \otimes I_n \right] = 
\left(
 \mathbf{M}^\top \bar{\Phi}  \mathbf{M} \right) \otimes I_n
$$
...
% = C+ \Big{ \{ }
% \mathrm{vec}(\mathbf{Y}) - \left[ \left( \mathbf{M}^\top \Phi^{\frac{1}{2}} \right) \otimes I_n \right] \mathrm{vec}(\mathbf{U}) \Big{\} } ^{\top}
%
%\left[ \left(
% \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top
%\right) \otimes I_n \right] 
%
%\Big{ \{ } \mathrm{vec}(\mathbf{Y}) -\left[ \left( \mathbf{M}^\top \Phi^{\frac{1}{2}} \right) \otimes I_n \right] \mathrm{vec}(\mathbf{U}) \Big{\}} 
 = C + 
%
...
\left[ \left( 
 \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top\right) \otimes I_n \right] 
\left[ \left( \mathbf{M}^\top \Phi^{\frac{1}{2}} \right) \otimes I_n \right] \,\mathrm{vec}(\mathbf{U})   \\
%
+  \mathrm{vec}(\mathbf{U})^{\top}
    \left[ \left(\Phi^{\frac{1}{2}} \mathbf{M} \right) \otimes I_n \right]
    \left[ \left(  \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top\right) \otimes I_n \right]
    \left[ (\mathbf{M}^\top\Phi^{\frac{1}{2}} ) \otimes I_n \right]
   \mathrm{vec}(\mathbf{U}) =\\
%
...
%
- 2\mathrm{vec}(\mathbf{Y})^{\top} 
\left[ \left(  \mathbf{M}^{-1} \bar{\Phi}^{-1} \Phi^{\frac{1}{2}}  \right)
...
\begin{align*}
q_{11} :=  \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top; \quad
q_{12} :=  \mathbf{M}^{-1} \bar{\Phi}^{-1} \Phi^{\frac{1}{2}}; \quad
q_{22} := \Phi \bar{\Phi}^{-1}
\end{align*}
Hence 
\begin{align*}
...
%
- 2\mathrm{vec} \,\mathbf{z}^{\top} 
\left(  q_{12} \otimes \mathbf{I}_n \right)  \,\mathrm{vec}(\mathbf{U})\\  
%
+  \mathrm{vec}(\mathbf{U})^{\top}
    \left(q_{12} \otimes \mathbf{I}_n + I_p \otimes L\right)
   \mathrm{vec}(\mathbf{U})
\end{align*}
...
\begin{equation} \label{eq:joint_bym_mmod}
    ...
    \sim N_{2np} \left( 0, \begin{pmatrix}
            ...
            \, - q_{12} \otimes I_n \\
            - q_{12}^\top\otimes I_n \, & \,
             q_{22} \otimes I_n + I_p \otimes \mathbf{L}
        \end{pmatrix}^{-1} \right)
\end{equation}
