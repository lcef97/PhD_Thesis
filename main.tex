...
%The Integrated Nested Laplace Approximation (INLA) is a computational method introduced by Rue, Martino and Chopin in 2009 \cite{INLA}; since then, one of its main fields of application has been spatial analysis \cite{INLArev} and it has proved to be a rather flexible tool for multivariate spatial modelling \cite{INLAMSM2}.\\ Considering the Latent Gaussian model outlined before, the key assumption behind the INLA is that $\theta |  \Psi$ is a Gaussian Markov Random Field (GMRF; see \cite{GMRFs}) with joint Normal distribution and sparse precision $\mathbf{Q}(\Psi)$. %
%%%%% THIS IS ONLY VALID IN THE CLASSIC MODE OF R-INLA; namely, if $m$ is the size of $\theta$ (in this case, $m = N + n + p$), then the number of nonzero entries in $\mathbf{Q}$ is in $\mathcal{\Theta}(m)$, i.e. in the same order of magnitude of $m$. The linear predictor is included in $\theta$ for computational reasons; with the appropriate choice of the stochastic representation of $\eta$ (see e.g. \cite{INLA2017}) if all other parameters are GMRFs, then $\theta$ is a GMRF as a whole. \\
...
%Territorial disparities in the Italian public education system are a severe and widely recognised issue. Exploratory analysis of school infrastructure endowment highlights a North-South divide encompassing several infrastructural dimensions, such as school accessibility and availability of learning and recreational spaces \citep{BDI}. The relevance of school infrastructure in learning processes is nowadays well documented \citep{WB}, hence it is reasonable to expect infrastructural disparities to be reflected in terms of student outcomes.

%However, comparing student outcomes across a country's complex geography is not a trivial question. To this aim, a framework to define standardised and spatially homogeneous indicators has been developed by the OECD throughout the Programme for International Students Assessment \citep[PISA,][]{PISA}. In Italy, this task is attributed by law \citep{InvalsiLaw} to the Institute for the Evaluation of the Education System (Invalsi hereinafted). The Invalsi Institute runs an anonymous skill evaluation test whose attendance is mandatory across all public schools in Italy, regarding four subjects: Italian, Mathematics, English reading, and English listening. For each subject, the Invalsi elaborates a student-specific score through a methodology akin to the one employed by the PISA.
...
%We propose to model the average Invalsi scores for Italian municipalities using spatial analysis. We employ the Invalsi census survey data, which the Institute publishes at different aggregation levels \citep{Invalsi_IS}, the most informative one being LAU units or municipalities. 
...
\subsection{Invalsi scores}\label{Par:Invalsi} The Italian Institute for the Evaluation of the Education System (INVALSI) is a research institute whose primary mission is to assess the quality of the public education system through a survey dedicated to the second and the last years of primary school, the last year of middle school, the second and the last years of high school. The Institute publishes both a sample survey including anonymized individual observations, and a census survey including the averaged scores at the region, province, and municipality levels. For privacy reasons, INVALSI only publishes the scores of the $978$ municipalities with at least two high schools \cite{Invalsi_IS}. Scores are expressed as the outcome of a psychometric model explaining the probability $\pi_{sq}$ that student $s$ passes the question $q$ (the same for all students) through two variables: student ability $b_s$ and item difficulty $d_q$ with such a formulation \citep[][Chapter 5]{PISA}:
 ...
 $$
The ability estimator $b_s$ is then scaled to have a global mean of 200 points and a global between-students standard deviation of 40 points.%; \textcolor{orange}{please notice that by construction this quantity is unbounded}. 
...
\begin{figure}
  \centering
  \includegraphics[width = 0.9\textwidth]{Invalsi_10_mun.pdf} 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{X_prov_1.pdf} \\%[1ex] 
...
%\begin{figure}
%  \centering
%  \includegraphics[width = 105mm]{eigen_pole.pdf} 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{X_prov_1_nosp_waic.pdf} \\%[1ex] 
...
% \label{eq:marginals}
% \begin{aligned}
% \pi(\theta_i | y) = \int_{\Psi} \pi(\theta_i | y, \Psi) \pi(\Psi | y) d \Psi\\
...
%\begin{equation}
%\pi_G(\theta | y, \Psi) \propto e^{\displaystyle {\frac{1}{2} (\theta - \mu_\Psi)' 
%\mathbf{H \mathrm{ln} f(\mu_\Psi, y | \Psi)} (\theta - \mu_\Psi)}}
%\label{eq:GaussianApprox}
%\end{equation}
%Where the $H$ operator denotes the Hessian matrix. Then, the joint posterior of hyperparameters $f(\Psi | y)$ can be computed, setting $\theta = \mu_\Psi$. 
...
%%$$
%%\mathrm{DIC} := P_D + \mathbb{E}[D(\theta, \Psi) \mid y]
%%$$
%%Where $$P_D = \mathbb{E}[D(\theta, \Psi)] - D(\hat{\theta}, \hat{\Psi})$$ denotes the effective number of parameters or unconstrained parameters and measures model complexity, $D(\theta, \Psi) = -2 \mathrm{ln} \pi(y \mid \theta, \Psi)$ being the model deviance, whose expectation measures the goodness of fit. Following \cite{INLA} we define $\hat{\theta}$ as $\mathbb{E}[\theta | y]$ and $\hat{\Psi}$  as the posterior mode of $\Psi$, this latter choice being due to the skewness in $\pi(\Psi | y)$. We show both the effective number of parameters and the expected deviance alongside the DIC.
 
%%The Watanabe-Akaike Information Criterion \cite{WAIC}, also known as Widely Applicable Information Criterion, is also useful in balancing model complexity and goodness of fit, the former being measured as $\mathrm{Var}[\mathrm{ln} \pi(y |\theta, \Psi)]$. An interesting feature of the WAIC is given by its averaging the likelihood over $\theta$ and $\Phi$ rather than using point estimates (\cite{GelmanWAIC}). We use the formulation provided by \cite{GelmanWAIC}. \\
 

%$$
%WAIC = 2 \sum_{j=1}^N VAR[\mathrm{ln} \pi(y_j |\theta, \Psi)]
%- 2 \sum_{j=1}^N  \mathrm{ln} \mathbb{E}[\pi(y_j | \theta, \Psi)]
%$$
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{yhat_mun_spatplus_waic.pdf} 
...
%\begin{equation}
%\pi(z \mid \Lambda, \phi) =
%\left( \frac{1}{2 \pi} \right)^{n}\sqrt{\mid \Lambda \otimes 
%(\mathbf{D} - \phi \mathbf{W})}  \mid \, \, 
 %e^{\displaystyle{-\tfrac{1}{2}
%\mathrm{vec}(z)'[ \Lambda  \otimes (\mathbf{D} - \phi \mathbf{W})] \mathrm{vec}(z)}}
%\label{eq:PCAR}
%\end{equation}
%%%%%%%%%%% Do not delete equation above %%%%%%%%%%%%%%%%%%%%
%
...
Results are similar to the ICAR, which is consistent with the finding that $\phi$ ranges close to one, denoting that spatial association is strong. %Lastly, in Fig. \ref{fig:zpcarprov} we plot the posterior expectation of $z$ under the PCAR model, applying the S+(5) correction.
...
%\begin{figure}
%  \centering
%  \includegraphics[width = 0.9\textwidth]{z_PMCAR_spatplus_waic.pdf} 
%  \caption{Expected spatial effects, province-level PCAR model with S+(5) correction}
%  \label{fig:zpcarprov}
%\end{figure}
