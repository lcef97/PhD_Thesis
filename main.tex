...
\begin{itemize}
\item The binary neighbourhood matrix $\mathbf{W}$, with $w_{ij} = 1 \Longleftrightarrow s_i \in \partial s_j$ and $w_{ij}=0$ otherwise; notice that by convention, $w_{ii}=0$ (cfr Chapter \ref{section:GMRFs})
\item The diagonal degree matrix $\mathbf{D}$, whose elements correspond to the number of edges of each node or, alternatively, to the row or column sums of $\mathbf{W}$, hence $d_i = \sum_{j=1}^{n} w_{ij} = \mathrm{card}(\partial s_i)$
\item The Laplacian matrix $\mathbf{L} =\mathbf{ D} - \mathbf{W}$, which is singular by construction and more precisely has rank deficiency equal the number of connected components in $\mathcal{G}$, \citep[see e.g.][]{ICAR, Hodges2003}
\end{itemize}
...
%
\begin{equation}
\pi(z  | \Sigma^{-1}) \propto -\frac{1}{2} (\mathrm{vec}_r\,z)^{\top} (\mathbf{L} \otimes \Sigma^{-1})\, \mathrm{vec}_rz
\label{eq:rowwise}
\end{equation}
%
...
$$
\mathcal{P} \left( \mathbf{L} \otimes \Sigma^{-1} \right) \mathcal{P}^{\top}= \Sigma^{-1} \otimes \mathbf{L}
$$
...
%
\begin{equation}
z|\Sigma \sim \mathcal{N}_{kn} \left( 0, \Sigma^{-1} \otimes \mathbf{L}^{+}\right)
\label{eq:icarjoint}
\end{equation}
%
...
Hence, by definition, ICAR variance incorporates a factor determined by the neighbourhood structure. At the global level, this consideration implies that process variability is not described by the precision parameter alone, since a deterministic scale factor concurs as well. The intuitive solution to this issue is to scale the ICAR model, as shown by \cite{Sorbye}. Following their operational proposal, we take the geometric mean of the diagonal of $\mathbf{S}$ as scaling factor - or, in other words, the geometric mean of marginal variances if all global precision parameters (diagonal entries of $\Sigma$) were equal to 1, namely
$$
...
or typical marginal variance). We are thus able to separate the parameter $\Sigma$ from the effect on variance induced by the graph structure. To do so, the scaled model does not employ $\mathbf{L}$ as the structure matrix, but the scaled matrix $\mathbf{L}_\mathrm{s} := \bar{\sigma}^2 \mathbf{L} $. On the other hand, the covariance matrix becomes $\bar{\sigma}^2 \Sigma$, which actually expresses the variability of the process independently of how the graph is structured. 
...
$$
\mathbf{L}_\mathrm{s} = \mathcal{P} \left( \bar{\sigma}_1^2 \mathbf{L_1} \oplus \bar{\sigma}_2^2 \mathbf{L_2}
\oplus ... \oplus \bar{\sigma}_G^2 \mathbf{L_G} \right) \mathcal{P^{\top}}
$$
Where $\mathcal{P}$ is an appropriate permutation matrix, $\mathbf{L_i}$ is the Laplacian of the $i$-th component of the graph, and $\bar{\sigma}_i^2$ is the relevant typical variance.


%% -------- More flexible areal models ------------%%
\section{Some extensions of the Intrinsic CAR} \label{section:CARs}

The ICAR model has a relatively small number of parameters, i.e. $p(p+1)/2$, corresponding to the entries of the precision matrix, and is easy to interpret. However, it can be easily seen that it is not a flexible model, in that it assumes a spatial autocorrelation parameter equal to $1$. It can be thought of as the spatial equivalent of $RW(1)$ processes in time series analysis: it is suited to detect strong spatial structures, but is misspecified when spatial trends are not particularly smooth. For this reason, some extension have been developed in literature. Here we introduce three popular ones, the so-called Proper CAR \citep{PCAR_Gelfand}, the Leroux proper CAR \citep{Leroux}, and the Besag-York-Mollié \citep[BYM][]{BYM, BYM2} convolution model. 
...
%
\begin{equation}
z|\Sigma, \alpha \sim \mathcal{N} \left( 0, \Sigma \otimes (\mathbf{D} - \alpha \mathbf{W})^{-1}\right)
 \label{eq:pcar}
\end{equation}
%
On a side note, this is a proper distribution, in that for $\alpha < 1$ the precision matrix is full-rank \footnote{For this reason, this model has been known in literature as the 'proper' CAR. Properness is far from an exclusive feature of this model, and is not even limited to the models listed in this section, yet this naming has become a convention. We respect this convention for notation brevity.}. One advantage of this model is the ease in interpretation, as $\alpha$ quantifies the extent to which the value of $z_j$ depends on the neighbouring sites. Clearly, the limit case for $\alpha = 1$ is the ICAR, while the limit case for $\alpha = 0$ is an independent but \textit{not} identically distributed model, in which the variance is proportional to the inverse degree matrix $\mathbf{D}^{-1}$, hence a site with a higher number of neighbours will have a smaller prior conditional variance.

\subsection{The Leroux proper CAR} \label{subsection:lcar}

As we have seen, a partial drawback of the PCAR model is that the limit case in absence of spatial autocorrelation is not an IID process. A simple solution would be to employ a rather similar model has been proposed by \cite{Leroux}. This time, the idea is to balance the precision matrix between the ICAR precision (proportional to the Laplacian) and the precision of an IID process (proportional to the identity matrix). To do so, a precision mixing parameter is introduced; here we label it as $\lambda \in [0,1]$. The starting point is the conditional prior:
%
$$
...
z|\Sigma, \lambda \sim \mathcal{N}_{pn} \left( 0,    \Sigma
  \otimes [\lambda \mathbf{L} + (1-\lambda)\mathbf{I}_n ]^{-1}\right)
 \label{eq:lcar}
\end{equation}
%
...
The original version of this model, proposed by \cite{BYM} was the convolution of a univariate ICAR, say $u' \sim \mathcal{N}=(0, \sigma_u^2 \mathbf{L})$, and a IID process, say $v \sim \mathcal{N}(0, \sigma_v^2 \mathbf{I})$. The underlying idea is to control for both spatial variation and random noise. This is particularly useful in disease mapping contexts, in which the dependent variable is usually Poisson distributed and the linear predictor needs a term to account for random noise, which is not captured by the IMCAR component alone. Some drawbacks can be found, however, in this model. The two components $u$ and $v$ are not separable and their convolution $z =: u' + v$ is what needs being modelled.

This formulation, however, hinders the interpretation of the extent to which process variability is driven either by spatial autocorrelation or by noise. For this reason, \cite{BYM2} proposed a reparametrisation of the BYM which allows to scale the variance; to do so, a variance mixing parameter $\phi \in [0,1]$ is introduced, and a unique scale parameter $\sigma^2$ is used in place of the two former parameters $\sigma_u^2$ and $\sigma_v^2$. The univariate scaled BYM model becomes thus:
$$
\left\{ \begin{array}{ll} 
z = \sigma \sqrt{\phi} u + \sigma \sqrt{1-\phi}v \\
u \sim \mathcal{N}_n \left(0, \mathbf{L}_{\mathrm{s}}^{+} \right)\\
v \sim \mathcal{N}_n(0, \mathbf{I}_n)
\end{array} \right.
$$
...
\begin{equation}
\text{LCAR:} \quad \quad 
\mathrm{Var}[\mathrm{vec} \,z | \Sigma, \Lambda] = (\mathbf{M^\top \otimes I_n})
 [\Lambda \otimes \mathbf{L} + (\mathbf{I}_p - \Lambda) \otimes \mathbf{I}_n]^{-1} (\mathbf{M} \otimes I_n)
\end{equation}
%
Regarding the BYM, throughout this thesis we will rely on the simplifying hypothesis that:
%
\begin{equation}
z = \mathbf{U} \Phi^\frac{1}{2} \mathbf{M} + 
\mathbf{V} (I_p - \Phi)^\frac{1}{2} \mathbf{M}
\label{eq:mmbym_stochrep}
\end{equation}
%
i.e. both the components share the same scale parameter. This is not as immediate as in the univariate case, in which the univariate scale parameter could be easily rescaled leveraging on the presence of the mixing parameter. This simplifying hypothesis is not admitted, e.g., by \cite{Corpas}, who did not employ the scaled BYM parametrisation and admitted a different scale parameter for each of the two BYM components. On the contrary, the unique $M$ approach is followed by \cite{Urdangarin24}, who indeed extend the parametrisation of \cite{BYM2} to the M-model case. We follow this approach as well. Equation \ref{eq:mmbym_stochrep} clearly implies
%
\begin{equation}
    \text{BYM:} \quad \quad
    \mathrm{Var}[\mathrm{vec} \, z | \Sigma, \Phi] = (\mathbf{M^\top \otimes I_n})
 \  [ \Phi \otimes \mathbf{L} + (\mathbf{I}_p - \Phi) \otimes \mathbf{I}_n]
 (\mathbf{M} \otimes I_n)
 \label{eq:mmbym}
\end{equation}
%
Lastly, in Appendix \ref{appendix:MMBYM} we show that sparse parametrisation is also possible for the multivariate BYM.
...
\subsection{Preliminary: Latent Gaussian model outline}\label{section:LGM}
...
\eta = A \vartheta \\
\vartheta | \Psi \sim \mathcal{N}(\mathbf{0}, (\mathbf{Q}_{\Psi})^{-1})
\end{array} \right. 
$$
where $y$ denotes the respondse variable consisting of $N$ observations; $\eta$ is the linear predictor linked with a function $g$ to the expected value of $y$; $\vartheta$ is a generic vector of latent variables of interest; $\sigma_{\varepsilon}^2 := \mathrm{Var}[\varepsilon]$ is the variance of the IID Gaussian error $\varepsilon$, $A$ is a known design matrix; $\Psi$ is the array of hyperparameters, and $\mathbf{Q}_{\Psi}$ denotes the precision matrix. %In the case of the multivariate ICAR, as seen in Section \ref{section:icar}, the precision is $\mathbf{Q}_{\Psi} = \Sigma^{-1} \otimes \mathbf{L}$, where $\Sigma$ is the scale parameter and $\mathbf{L}$ is the graph Laplacian matrix. For the univariate case, the precision parameter clearly reduces to a positive scalar.
...
%in order to map each observed value $y_j$ to one realisation of the random effect $z_i$, i.e. if $y_j$ depends on $z_i$ and therefore $\xi_{ji} = 1$, then  $y_j \perp\!\!\!\perp  z_{-i}$, with $i \in [1, n]$ and $j \in [1, N]$, where each row of $\xi$ has exactly one entry equal to $1$ and all other entries equal to $0$. 
 ...
 For illustration purposes, let us now assume that $y$ is Gaussian and the linear predictor includes a univariate Gaussian latent effect of size $n$ with prior mean $0$ and non-negative defined precision matrix $\mathbf{Q}(\Psi) = \tau \mathbf{R}$, where $\mathbf{R}$ is a semidefinite positive structure matrix and $\tau$ is a global precision parameter; for the ICAR case, clearly $\mathbf{R} \equiv \mathbf{L}$. In such Gaussian regression framework, the model becomes:
%
\begin{equation}
%\begin{aligned}
\left\{ \begin{array}{ll} 
    y = X \beta + \xi z + \varepsilon \\
    z \sim \mathcal{N} (0, \tau^{-1} \mathbf{R}^{-1}) \\
...
C^{-1} = \tfrac{\tau}{\tau_{\varepsilon}} \begin{pmatrix}
 \frac{1}{\tau M} \, I_p & 0_{p \times n} \\ 0_{n \times p} & \mathbf{R}
\end{pmatrix}
$$

Since the error term $\varepsilon$ and the latent effects $z$ have different precision parameters, the matrix $C^{-1}$ cannot be considered known. To see this, consider the prior precision of the random effects defined as $\tau \mathbf{R}$; $\tau_{\varepsilon} C^{-1}$ must thus be structured as $\begin{pmatrix} \frac{1}{M} \, I_p & 0_{p \times n} \\ 0_{n \times p} & \tau \mathbf{R} \end{pmatrix}$ where $M$ is the known variance parameter of $\beta$, but $\tau$ is unknown and isolating $\tau$ to express $\tau_{\varepsilon} C^{-1} = \tau \begin{pmatrix}
 \frac{1}{\tau M} \, I_p & 0_{p \times n} \\ 0_{n \times p} & \mathbf{R}
...
\begin{pmatrix}
 ...
 \xi ^{\top} X & \xi^{\top} \xi + \bar{r} \mathbf{R}
\end{pmatrix}^{-1} = \\ = 
\begin{pmatrix}
...
\end{equation}
%\begin{pmatrix}
%G^{-1} + G^{-1} X^{\top} \xi(\xi ^{\top} \xi + \bar{r} \mathbf{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1} \xi ^{\top} XG^{-1} %& 
%-G^{-1} X^{\top} \xi(\xi ^{\top} \xi + \bar{r} \mathbf{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1}\\
%-  (\xi ^{\top} \xi + \bar{r} \mathbf{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1} \xi^{\top} X G^{-1} & 
% (\xi ^{\top} \xi + \bar{r} \mathbf{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)^{-1}
%\end{pmatrix}

Where $S_{11} := (\xi ^{\top} \xi + \bar{r} \mathbf{R} - \xi ^{\top} X G^{-1} X^{\top} \xi)$ is the Schur complement of the upper-left block in $\left(A_1^\top A_1 + C^{-1}\right)^{-1}$, $\bar{r} := \dfrac{\tau}{\tau_{\varepsilon}}$ and $G := (X^{\top}X + r_{\varepsilon}I_p)$, where $r_{\varepsilon} := \dfrac{1}{\tau_{\varepsilon}M} = \dfrac{\sigma_{\varepsilon}^{2}}{M}$.
...
Equivalently, if for brevity we define $\Gamma = \xi ^{\top} \xi + \bar{r} \mathbf{R} - \xi^{\top} X G^{-1} X^{\top} \xi$ this means that the hat matrix equals:
\begin{equation}
\begin{aligned}
...
$$
\ln \pi(z | \tau) = C -\dfrac{\tau}{2} z^{\top} \mathbf{R} z  = 
C -\dfrac{\tau}{2} z^{\top} (E \, K) \begin{pmatrix} E^{\top} \\ K^{\top}\end{pmatrix} 
\mathbf{R} (E \, K) \begin{pmatrix} E^{\top} \\ K^{\top}\end{pmatrix} z 
$$
...
$$
\zeta \sim \mathcal{N} \left(\begin{pmatrix} 0 \\ 0 \end{pmatrix}, \, \tau^{-1} \begin{pmatrix} E^{\top}\mathbf{R}E & E^{\top}\mathbf{R}K \\ K^{\top}\mathbf{R}E & K^{\top}\mathbf{R}K \end{pmatrix}^{-1} \right)
$$
...
%Under the unrestricted model, we have $\mathrm{Prec}[\zeta_1 | \tau, K^{\top}z = 0] = E^{\top}\mathbf{R}E$, as in \citep[eq. 16-17 at p. 1201][]{RHZ}. To see this, recall:
%$$
%\ln p(\zeta | \tau; K^{\top}z = 0) \propto 
% -\dfrac{\tau}{2}  (\zeta_1^{\top} \, 0_{p}^{\top}) \begin{pmatrix} E^{\top} \\ K^{\top}\end{pmatrix} 
%\mathbf{R} (E \, K)  \begin{pmatrix} \zeta_1 \\ 0_{p} \end{pmatrix} = -\dfrac{\tau}{2} \zeta_1^{\top} \, E^{\top}\mathbf{R}E \, %\zeta_1
%$$
...
As a side effect of deconfounding $z$, it may be worth noticing that the the size of the new precision matrix is $(n-p) \times (n-p)$ and its rank is $ \leq \mathrm{min} \lbrace rk(E), rk(\mathbf{R}) \rbrace = \mathrm{min} \lbrace (n-p), rk(\mathbf{R}) \rbrace$. Hence, if the rank deficiency of $\mathbf{R}$ is less than $p$, the new spatial effect has a proper distribution even if $z$ is an ICAR process. If the underlying graph is disconnected and $p(z)$ can be factorised in a factor for each connected component (CC) then the rank deficiency of $\mathbf{R}$ is equal to the number of CCs, but it is necessary to impose a sum to zero constraint for each CC, hence the same number of intercepts would be required; thus, also in this case $n-p$ is smaller than $rk(\mathbf{R})$ and $E^{\top}\mathbf{R}E$ is still proper. However, there are still some possible situations in which this inconvenience can arise, e.g. in multilevel models, like in Chapter \ref{Chapter:Invalsi}.
...
$$
Where $\beta_\mathrm{RSR}$ denotes the vector of covariate effects under restricted spatial regression and $\beta_{OLS}$ are covariate effects estimated via the ordinary least squares. This implies $\beta$ is estimated independently of the latent effect, and its expectation equals that of the nonspatial model. Holding a flat prior on $\beta$, referring to equation \ref{eq:projmat}, we see that under the  unrestricted model, with $\xi = I_n$, $\Gamma = \bar{r} \mathbf{R} + P_{\bot}$
\begin{align*}
H_{\tau_{\varepsilon}, \bar{r}} = P + (\bar{r} \mathbf{R} + P_{\bot})^{-1}
- P (\bar{r} \mathbf{R} + P_{\bot})^{-1} - (\bar{r} \mathbf{R} 
+ P_{\bot})^{-1} P + P (\bar{r} \mathbf{R} + P_{\bot})^{-1}P = \\
= P + P_{\bot} (\bar{r} \mathbf{R} + P_{\bot})^{-1} P_{\bot}
\end{align*}
...
To gain some context, let us first recall that the Moran index $I(z)$ referring to a vector $z$ of length $n$ taking values over a graph with neighbourhood matrix $\mathbf{W}$ is given by 
%
\begin{equation}
I(\mathbf{W}) = \frac{n}{\mathbf{1^\top W 1}} \frac{z^\top \mathcal{C} \mathbf{W} \mathcal{C}z}{z^\top \mathcal{C}z}
\label{eq:I_moran}
\end{equation}
%
where $\mathcal{C}$ is the idempotent centering matrix $\mathcal{C}:=\mathbf{I}_n - \frac{1}{n}\mathbf{11}^\top$ implying  $\mathcal{C}z = z - \bar{z}$. 

\paragraph{Smoothing orthogonal to the space of covariates by spatial filtering}

%is basically the numerator of the Moran coefficient \citep{Cliff_Ord} and for a variable $x$ taking values over a graph whose neighbourhood matrix is labelled as $\mathbf{W}$ we have $\mathcal{M}x := (x - \bar{x})^\top \mathbf{W} (x - \bar{x}) 1 = x^\top(\mathbf{I}_n - \frac{1}{n}\mathbf{11}^\top) \mathbf{W} (\mathbf{I}_n - \frac{1}{n}\mathbf{11}^\top)x$, where $\mathbf{1}$ is a vector of ones of appropriate length
...
\cite{HughesHaran} introduce what they refer to as the \emph{Generalised Moran operator for $\mathbf{X}$ with respect to the graph}, i.e.
%
$$
I_{\mathbf{X}}(\mathbf{W}) =  \frac{n}{\mathbf{1^\top W 1}} \frac{z^\top P_\bot \mathbf{W} P_\bot z}{z^\top P_\bot z}
$$
%
...
As mentioned in section \ref{subsection:Mmodels}, we use a unique scale parameter $\Sigma$ for the two components, and define $\mathbf{M}$ as a generic full-rank matrix such that $\mathbf{M}^{\top}\mathbf{M} = \Sigma$. For convenience, we also define the precision parameter $\mathbf{T} =: \Sigma^{-1}$.

Consider the model in equation \ref{eq:mmbym}. Keeping up with the approach of \cite{BYM2}, and defining for the sake of brevity $\bar{\Phi} := \mathbf{I}_p - \Phi$, we know that:
%
$$
\mathbb{E} [\mathrm{vec}\, \mathbf{z} \mid U, \Phi , \Sigma] = \mathrm{vec}(\mathbf{U} \Phi^{\frac{1}{2}} \mathbf{M}) =
[( \mathbf{M}^{\top}\Phi^{\frac{1}{2}}) \otimes \mathbf{I}_n] \mathrm{vec}\,\mathbf{U}
$$
%
and similarly 
%
$$
\mathrm{Var} [\mathrm{vec}\, \mathbf{z} \mid \mathbf{U}, \Phi , \Sigma] =
\left[( \mathbf{M}^{\top} \bar{\Phi}^{ 1/2}) \otimes \mathbf{I}_n \right] 
\mathbb{E} \left[
\mathrm{vec}\mathbf{V}
(\mathrm{vec}\mathbf{V})^\top \right]
\left[(\bar{\Phi}^{1/2} \mathbf{M}) \otimes \mathbf{I}_n \right] = 
\left(
 \mathbf{M}^\top \bar{\Phi}  \mathbf{M} \right) \otimes \mathbf{I}_n
$$
%
The distribution of $\mathbf{Y} \mid \mathbf{U}, \Sigma, \Phi $ then reads:
\begin{align*}
-2 \ln \pi \left(\mathrm{vec} \, \mathbf{Z} \mid \mathbf{U}, \Sigma, \phi \right) =\\
 %
=(\mathrm{vec}\,\mathbf{Z})^{\top}
\left[ \left(
 \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top
\right) \otimes \mathbf{I}_n \right] 
\mathrm{vec}\,\mathbf{Z}+ \\ 
%
- 2(\mathrm{vec}\,\mathbf{Z})^{\top} 
\left[ \left( 
 \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top\right) \otimes \mathbf{I}_n \right] 
\left[ \left( \mathbf{M}^\top \Phi^{\frac{1}{2}} \right)
       \otimes \mathbf{I}_n \right] \,\mathrm{vec} \, \mathbf{U}  + \\
%
+  (\mathrm{vec}\,\mathbf{U})^{\top}
    \left[ \left(\Phi^{1/2} \mathbf{M} \right) \otimes \mathbf{I}_n \right]
    \left[ \left(  \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top\right)
          \otimes \mathbf{I}_n \right]
    \left[ (\mathbf{M}^\top\Phi^{\frac{1}{2}} ) \otimes \mathbf{I}_n \right]
   \mathrm{vec}\,\mathbf{U} =\\
%
= C + (\mathrm{vec}\,\mathbf{Z})^{\top}
\left[ \left(
 \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top
\right) \otimes \mathbf{I}_n \right] 
\mathrm{vec}\,\mathbf{Z} +\\ 
%
- 2(\mathrm{vec}\,\mathbf{Z})^{\top} 
\left[ \left(  \mathbf{M}^{-1} \bar{\Phi}^{-1} \Phi^{1/2}  \right)
\otimes \mathbf{I}_n \right]  \,\mathrm{vec}\,\mathbf{U}  +\\
%
+  (\mathrm{vec}\,\mathbf{U})^{\top}
    \left[ \left(  \Phi  \bar{\Phi}^{-1}  \right) \otimes \mathbf{I}_n \right]
   \mathrm{vec}\,\mathbf{U}
\end{align*}
%%
Now, for brevity let us define the following $p \times p$ matrices:
\begin{align*}
\mathbf{q}_{11} :=  \mathbf{M}^{-1} \bar{\Phi}^{-1}{\mathbf{M}^{-1}}^\top; \quad
\mathbf{q}_{12} :=  \mathbf{M}^{-1} \bar{\Phi}^{-1} \Phi^{\frac{1}{2}}; \quad
\mathbf{q}_{22} := \Phi \bar{\Phi}^{-1}
\end{align*}
Hence 
\begin{align*}
-2 \ln \pi \left(\mathrm{vec}\,\mathbf{z} \mid \mathbf{U}, \Sigma, \Phi \right) 
= C + \mathrm{vec}(\mathbf{z})^{\top}
\left(\mathbf{q}_{11} \otimes \mathbf{I}_n \right) 
\mathrm{vec}\,\mathbf{z} +\\
%
- 2(\mathrm{vec}\,\mathbf{z})^{\top} 
\left(  \mathbf{q}_{12} \otimes \mathbf{I}_n \right)  \,\mathrm{vec}\,\mathbf{U}+\\
%
+  (\mathrm{vec}\,\mathbf{U})^{\top}
    \left(\mathbf{q}_{22} \otimes \mathbf{I}_n \right)
   \mathrm{vec}\,\mathbf{U}
\end{align*}
And
\begin{align*}
-2 \ln \pi \left(\mathrm{vec} \,\mathbf{z}, \mathrm{vec}\,\mathbf{U} \mid \Sigma, \phi \right)  
= C + (\mathrm{vec}\,\mathbf{z})^{\top}
\left(\mathbf{q}_{11} \otimes \mathbf{I}_n \right) 
\mathrm{vec}\,\mathbf{z} +\\ 
%
...
%
+  (\mathrm{vec}\,\mathbf{U})^{\top}
    \left(\mathbf{q}_{12} \otimes \mathbf{I}_n + \mathbf{I}_p \otimes L\right)
   \mathrm{vec}\,\mathbf{U}
\end{align*}
Hence, with some straightforward algebra, it can be concluded that:
\begin{equation} \label{eq:joint_bym_mmod}
    \begin{pmatrix}
        \mathrm{vec}\, \mathbf{z} \\ \mathrm{vec} \,\mathbf{U} 
    \end{pmatrix}
    \sim N_{2np} \left( 0, \begin{pmatrix} 
            \mathbf{q}_{11} \otimes \mathbf{I}_n \, & 
            \, - \mathbf{q}_{12} \otimes \mathbf{I}_n \\
            - \mathbf{q}_{12}^\top\otimes \mathbf{I}_n \, & \,
             \mathbf{q}_{22} \otimes \mathbf{I}_n + \mathbf{I}_p \otimes \mathbf{L}
        \end{pmatrix}^{-1} \right)
\end{equation}
Which consists of four blocks; three are diagonal and the lower-right one has the same zeroes pattern as $\mathbf{L}$.
...
To see this, consider a generic $n$-dimensional Gaussian random field $\mathbf{X} = (X_1, X_2 .. . X_n) \sim \mathcal{N}(\mathbf{\mu}, \mathbf{\Sigma})$. Then, a deductive prove is given for this property:
\begin{equation}
		X_i \bot X_j \mid \mathbf{X_{-ij}} \iff \mathbf{q}_{ij} = 0
		\label{eq:theorem1}
\end{equation}

For any $i \neq j,  \quad i,j \in [1,n]$; $\mathbf{q}_{i,j}$ is the element in position $(i,j)$ in the precision matrix $\mathbf{Q}: = \mathbf{\Sigma^{-1}}$, assuming that $\mathbf{\Sigma}$ is nonsingular. Any set of indices $A \subset \mathbb{N}^n$ can be chosen in order to partition $\mathbf{X}$ into $\mathbf{X_A}$ and $\mathbf{X_B}$, being $B = \mathbb{N}^n / A$ with $card(A) := m<n, \, card(B) = n-m$, namely:
	$$
...
Therefore it is proved that, if $\mathbf{X} \sim \mathcal{N}(\mathbf{\mu}, \mathbf{\Sigma})$, for any two partitons $A$ and $B$ such that $\mathbf{X_B}$ is the complement to $\mathbf{X_A}$ with respect to $\mathbf{X}$, then 

\begin{equation}
	\mathbf{X_A} | \mathbf{X_B} \sim \mathcal{N} \left(
...
\begin{equation}
	\mathrm{COV}[X_i, X_j | \mathbf{X_{-ij}}] \equiv \mathbf{q}_{ij} 
\end{equation}
...
(\vartheta - \mu_0)\mathbf{q}_0(\vartheta - \mu_0)
}
\label{eq:apprG}
\end{equation}
where $\mu_0 = \vartheta_0(y, \Psi)$ and $\mathbf{q}_0 = -H g(\vartheta_0(y, \Psi))$. 
...
\pi_{VB}(\vartheta | y, \Psi) \propto e^{\displaystyle{
 - \frac{1}{2}(\vartheta - \mu_0 - \Sigma_I \lambda)'\mathbf{q}_0(\vartheta - \mu_0 - \Sigma_I \lambda)
}}
$$
where $\Sigma_I$ is a projection matrix determined from a subset of the columns of $\mathbf{q}_0^{-1}$. Now, $\lambda$ is determined as the minimum to the following objective function:
\begin{equation}
\begin{aligned}
...
% \centering
%\begin{tabular}{lrrrr}
% & mean & s.d. & $\mathbf{q}_{0.025}$ & $\mathbf{q}_{0.975}$ \\
% \hline
...
\begin{equation}
\pi\left(z\mid \mathbf{T} \right) =
\left( \frac{1}{2 \pi} \right)^{n-3}
\sqrt{\mid \mathbf{T} \otimes\mathbf{L}  \mid_{+}} \, \, 
 e^{\displaystyle{-\tfrac{1}{2}
(\mathrm{vec} \, \mathbf{z})^\top(\mathbf{T} \otimes \mathbf{L} ) \mathrm{vec}\, \mathbf{z}}}
\label{eq:ICAR}
\end{equation}

where $\mathbf{L} := \mathbf{D} - \mathbf{W}$ is the Laplacian matrix of the graph $\mathcal{G}$ with $3$ connected components and $\mathbf{D} = \mathrm{diag}(d_1, d_2 \ldots d_n)$ is the degree matrix of $\mathcal{G}$ (as discussed more in detail in Chapter \ref{chapter:theory}). The matrix $\mathbf{T} = \Sigma^{-1}$ is the $2 \times 2$ precision parameter. 
Since $\mathbf{L}$ is singular with rank deficiency $3$ \citep{Hodges2003}, it is necessary to constrain $z$ to sum to zero within each connected graph component \citep{ICAR}. 
This is the reason for adopting component-specific intercepts $\beta_C$ in equation \ref{eq:model}.

To ease the interpretation of $T$ as the precision parameter of $z$, it is possible to cleanse it from the effect of the neighbourhood structure by defining a scaled version of $\mathbf{L}$ \cite{Sorbye} and reparametrising the precision of $z$ accordingly. The scaling procedure is described in subsection \ref{subsection:scaling}.
Since $\mathcal{G}$ is disconnected, each component-specific block of $\mathbf{L}$ is multiplied by the relevant typical variance, namely the geometric mean of the diagonal of the corresponding block of its pseudoinverse, following the methodology proposed by \cite{Freni}. It is therefore possible to define a precision parameter $T_\mathrm{scaled}$ which is not confounded with graph-induced effects. The scaled precision is assigned a Wishart prior \citep{Gelman} with $2k+1$ degrees of freedom and scale parameter equal to the identity matrix, i.e. $T_\mathrm{scaled} \sim \mathrm{Wishart}_{k}( I_k, 2k+1)$, with $k=2$ \citep{INLAMSM}.
...
$$
\mathbf{L} = \mathbf{V \Gamma V}^{\top}
$$
where the eigenvalues in $\mathbf{\Gamma}$ are in decreasing order and the eigenvectors in $\mathbf{V}$ have a decreasing number of oscillations. 
...
For the present data, the Spatial+2.0 methodology implies a shift in the value of explanatory variables by an additive correction defined at the macro-area level, namely $\mathbf{\bar{X}^{(S)}} + \mathbf{\bar{X}^{(0)}}$. For instance, for a generic $h$-th covariate the corresponding deconfounded values are $x_{i;h} -x_{i;h}^{(S)}-x_{i;h}^{(0)}$. Subsequently, to allow model comparisons addressed in Section \ref{Par:results}, deconfounded covariates are scaled to have the same global variance as the input ones.
...
In Figure \ref{fig:eigen_prov} we plot the two lowest-frequency eigenvectors of $\mathbf{L}$ at the macro-area level of provinces and varying across the continent. Eigenvectors are ordered decreasingly with respect to corresponding eigenvalues. It is possible to see that the 101st eigenvector follows a quadratic trend with one oscillation, whereas the 102nd eigenvector follows a linear North-South trend.  
