...
%%
\title{Statistical areal methods to map territorial vulnerability}
...
We prove in Appendix \ref{appendix:icar} that the joint distribution of a multivariate ICAR field is:
%
\begin{equation}
...
$$
\bar{\sigma}^{2} = \prod_{i=1}^{n} \sigma_{ii}^{2/n_z}
$$
...
%$ \displaystyle{\frac{1}{\bar{\sigma}^2}} \Lambda$, and this is what actually expresses the precision of the process independently of how the graph is structured. 
...
%A different approach to deal with spatial confounding, introduced by \cite{Dupont}, is based on adjusting the covariates rather than constraining spatial effects by representing the former as the sum of a spatial and a nonspatial component; it is known in the literature as Spatial+.% While Spatial+ was originally proposed for thin plate spline regression, 
...
\subsubsection{TBD: Spatial+}
\subsubsection{TBD: Spatial+2.0}
%
%% ------------------------------------------------------------------------------ %%
%% --------                   Second Chapter                 --------------------- %%
%% ------------------------------------------------------------------------------ %%
...
As an alternative strategy, we removed the smallest number of eigenvectors for $\boldsymbol{\bar{X}^{(NS)}}$ not to display evidence of autocorrelation according to Moran's $I$ index. %\textcolor{red}{, whose use is encouraged by its robustness to non-Normality \cite{Griffith} in datasets with at least $100$ observations.%} 
