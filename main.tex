...
Before criticism against RSR, several alternative approaches have being proposed in the last twenty years. One of the earliest ones was introduced by \cite{HughesHaran} and can be thought of as a refinement of RSR. In brief, they noticed the constraints used in RSR are not informative on the spatial structure of data, hence they developed a restriction methodology based on the eigenvectors of a generalised Moran operator, rather than the projection matrix. This is particularly interesting in that the collinearity between the unmeasured confounder and the explanatory variables is ruled out by means of a spatial filter. As an example, \href{https://github.com/lcef97/PhD_Thesis/blob/main/docs/Media/Moran_eigenvectors.gif}{in this gif} we show the structure of the 30 eigenvectors associated with the largest eigenvalues of a Moran operator defined on a set of 4 randomly generated Uniform variables over a $30 \times 30$ regular lattice; the neighbourhood matrix in this case follows the queen's criterion (neighbours share either an edge or a vertex).
...
We start by assuming that each \textcolor{red}{element} of an $n$-dimensional Gaussian random field can thus be associated with a node in a graph defined as at the beginning of section \ref{section:icar}.
...
$$  %% = \frac{\pi(y)}{\pi(y_{-j})}
Low CPO values denote "surprising" observations and, hence, possible outliers. Details on how the CPO is computed in \texttt{R-INLA} are provided by \cite{CPOINLA}. Here, we compare models through the LPML \citep{Geisser}, i.e.  $\mathrm{LPML} :=\displaystyle{- \sum_{h=1}^2 \sum_{j = 1}^N \mathrm{ln}\, \mathrm{CPO}_{j,h}}$.
